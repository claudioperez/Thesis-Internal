...
\item
  ...
  the \emph{internal operators} of Box 2. % which are internal to the element and are identified as the \emph{internal operators}. 
...
\item
  ...
  was in deriving Equation~(\ref{eq:def-pa}). This effect is
...
\hypertarget{internal-operators}{%
...
Equation~(\ref{eq:bar-interp}) by Equation~(\ref{eq:eta-parametric}) and
Equation~(\ref{eq:uubar-parametric}):
%
\begin{equation*} 
...
\(\left\{{\bm{\varphi}}^{}_{u a}\right\}\) for Equation~(\ref{eq:interp}) are:
%
\begin{equation}
...
Equation~(\ref{eq:para-bases}) in the formulas of Box 2. If the Petrov-Galerkin
approach is taken, only the transformation of Equation~(\ref{eq:uubar-parametric})
...
Section~\ref{sec:element-cj2}. Note that the discrete equilibrium operator
given by \(\mathbb{B}^*\bm{\varphi}_{\eta}\) will be identical to that
of the simple formulation of Section~\ref{sec:element-svq}.
Moreover, the geometric stiffness differs from 
the simple formulation only by post-multiplication with
\(g^*\). Consequently, formulating an element with
...
\(\bm{B}^\star_b\). This generally results in a more involved expression for the 
...
Propagating Equation~(\ref{eq:adapt-displ}) through the characteristic operators
of Box 2 shows that the nodal operators factor out, and the effect on
...
\end{equation}
where $\bar{\mathbf{k}}_{ab}$ is the stiffness resulting from substitution of Equation~(\ref{eq:para-bases}) into the operators of Box 2,
...
the nodal transformations in Equation~(\ref{eq:adapt-displ}): 
\begin{equation}
{\bm{\varphi}}^{}_{\eta a} = g^*\bar{\bm{\varphi}}^{}_{\eta a} \, \mathbf{a}_a .
\label{eq:adapt-eta}
\end{equation}
Using Equation~(\ref{eq:B-assoc}) and the definition of \(\bm{p}_a\) given by Equation~(\ref{eq:def-pa}),
...
of Equation~(\ref{eq:adapt-eta}) are equivalent to externally-applied transformations with the form:
\begin{equation} 
\begin{aligned}
...
\end{equation}
where $\bar{\bm{p}}_{a}$ is the resisting force resulting from substitution of Equation~(\ref{eq:para-bases}) into the operators of Box [TODO],
...
rederiving
the operators in Box 2 using Equation~(\ref{eq:adapt-eta}).
...
Equation~(\ref{eq:motion}) as follows: 
\begin{equation}
\begin{aligned}
...
% Equation~(\ref{eq:R-director}) by the sum \(\dot{\bar{\FrameBasis}}_k\otimes\FrameBasis_k\),
...
% by Equation~(\ref{eq:hat}). Using this in
Equation~(\ref{eq:isom-dxe}) furnishes: %
\begin{equation*} 
\begin{aligned}
...
\vspace{5ex}
\begin{ambox}[Corotational Transformation]{box:corotation}
\begin{enumerate}
    \item 
\[
...
\bm{x}(\xi),\FrameRotation(\xi) \mapsto \begin{pmatrix}
    \bm{x}(\xi) \\
    \operatorname{Log}(\FrameRotation (\xi))
\end{pmatrix}
\]
\end{enumerate}
\end{ambox}
...
The split form of the inner product in Equation~(\ref{eq:conf-pair}) allows
Equation~(\ref{eq:galerkin}) to be written in terms of components
...
same form as Equation~(\ref{eq:global-local}), and \(\mathcal{G}\) follows: %
\begin{equation*} 
 \begin{aligned}
...
strain operator as suggested by Equation~(\ref{eq:vstrain}).
When indeed \(\mathbb{B}^*\bm{\psi} = \mathbf{0}\), Equation~(\ref{eq:galerkin-local}) implies %suggests
...
\end{equation}
Using Equation~(\ref{eq:dpsidu}) with Equation~(\ref{eq:psi}) furnishes:
\begin{equation*} 
\begin{aligned}
...
\end{aligned}
\end{equation*}
Finally, the linearization of Equation~(\ref{eq:rottan}) gives for the rotation:
%
\begin{equation*} 
\begin{aligned}
...
Equation~(\ref{eq:global-local}), %and Equation~(\ref{eq:local-u}) 
...
%$\mathbf{k}_{a b}$ % by Equation~(\ref{eq:adapt-petr-para}) or Equation~(\ref{eq:adapt-petr-isom}), and optionally Equation~(\ref{eq:adapt-para-bubn}) or Equation~(\ref{eq:adapt-isom-bubn}) when a Bubnov-Galerkin formulation is adopted. 
...
% Application of Equation~(\ref{eq:adapt-petr-para})
% is required to maintain a consistent tangent stiffness, while Equation~(\ref{eq:adapt-para-bubn})
...
\item \emph{Kinematic update}:
  ...
  \end{equation*}
  which takes the form of Equation~(\ref{eq:adapt-petr-para}) in the parametric case, and Equation~(\ref{eq:adapt-petr-isom})
...
\item \emph{Conjugate update}: 
  \begin{equation*}
    ...
    \end{array}
  \end{equation*}
  which takes the form of Equation~(\ref{eq:adapt-para-bubn}) in the parametric case, and Equation~(\ref{eq:adapt-isom-bubn})
  in the isometric case.
\end{enumerate}
\end{ambox}
