...
The constraints on this map are identical to those of Equation~(\ref{eq:def-SO3})
...
\,\right\}
\end{equation*}
where the right representation \(T_{\scriptscriptstyle{\Lambda}}^r \mathrm{SO}(3)\) of the tangent space on \(\mathrm{SO}(3)\) is used (Equation~\ref{eq:rTSO3}).
Leveraging the inner product on \(\mathrm{SO}(3)\)
in Equation (\ref{eq:def-SO3-inner}), the associated inner product on
...
\begin{equation} 
\left\{\begin{array}{r}
\shearResult^{\prime}+{\bm{f}}_n=\bm{0} \\
\curveResult^{\prime}+\bm{x}^{\prime} \times \shearResult+{\bm{f}}_m=\bm{0} 
\end{array}\right. .
\end{equation}
...
\begin{equation*}
  \begin{aligned}
    ...
    &= \bm{\varLambda}\left( N\FrameBasis +  V_\alpha \FrameBasis_\alpha \right) \\
  \end{aligned}
  \qquad \text{ and }\qquad
  \begin{aligned}
    \curveResult
    &= T \frameBasis + M_\alpha \frameBasis_\alpha \\
    &= \bm{\varLambda}\left( T\FrameBasis + M_\alpha \FrameBasis_\alpha \right) \\
  \end{aligned}
\end{equation*}
...
of Equation~(\ref{eq:strong}) with:
\begin{equation} 
  \mathbb{B} \triangleq -\begin{bmatrix}
    \frac{\mathrm{d}}{\mathrm{d}\xi}\bm{1} &  \\
    \bm{x}^{\prime\times} & \frac{\mathrm{d}}{\mathrm{d}\xi}\bm{1}
  \end{bmatrix}
...
\label{eq:cosserat-B}
\end{equation}
where $\bm{1}$ is the $3 \times 3$ identity and the notation $(~\cdot~)^{\times}$
is introduced in Equation~(\ref{eq:hat}).
%
...
\right\}
$$
as presented by \textcite{reissner1973onedimensional}. Note however that certain signs differ owing to his peculiar notation for the components of \(\curveResult\). That is, from his Equation~3b the components \(M_1\) and \(M_2\) are instead defined by:
$$
\begin{aligned}
...
Applying the inner product Equation~(\ref{eq:rod-inner}) to the equilibrium Equation~(\ref{eq:cosserat-B}) furnishes
\begin{equation*}
  \begin{aligned}
...
\begin{equation}
  ...
  \mathbb{B}^* \triangleq \begin{bmatrix}
  \frac{\mathrm{d}}{\mathrm{d} \xi}\bm{1} & \bm{x}^{\prime \times}  \\
  \bm{0} & \frac{\mathrm{d}}{\mathrm{d} \xi} \bm{1}
  \end{bmatrix}.
\label{eq:def-Bstar}
\end{equation}
%
...
furnish components representing axial \(\epsilon\), shear \(\gamma_{\alpha}\), torsion \(\tau\),
...
\begin{equation*} 
  ...
  \begin{aligned}
    ...
    &= \tau \frameBasis + \kappa_\alpha \frameBasis_{\alpha}
  \end{aligned}
\end{equation*}
%
...
Equation~(\ref{eq:cosserat-pull}) for $\bm{A}$ furnishes the material stress $\bm{s}$ and strain $\bm{e}$:
\begin{equation} 
...
\item One may consider the alternative rotation \(\SpaceRotation\) defined by
  \begin{equation*}
    \begin{aligned}
    \FrameRotation &\triangleq \frameBasis_k \otimes \FrameBasis_k \\
    \SpaceRotation &\triangleq \frameBasis_k \otimes \SpaceBasis_k \\
    \bm{R}_0 &\triangleq \FrameBasis_k \otimes \SpaceBasis_k \\
    \end{aligned}
    \quad\text{ so }\quad 
    \FrameRotation = \SpaceRotation\SpaceRotation_0^{\mathrm{t}}
\end{equation*}
\item \label{rem:exact-shear-deriv} Note that in components one has for the derivative \((~\cdot~)'\) with respect to the reference length \(\reflenx\):
  $$
  \curveResult^{\prime}
   = (M_i^\prime + e_{ijk}\kappa_j M_k)\frameBasis_i 
  $$
  and the moment equilibrium can be expressed correspondingly as:
  $$
  \frameBasis_i (M_i^\prime + e_{ijk}(\kappa_j M_k + \gamma_j N_k))
  $$

\item With respect to the current arc length \(\deflenx\)
    \begin{equation*}
    \frac{\mathrm{d} \curveResult}{\mathrm{d} \reflenx}
    =\frac{\mathrm{d} \deflenx}{\mathrm{d} \reflenx}\left(\frac{\mathrm{d} M_k}{\mathrm{d} \deflenx} - \kappa^{\times}_{k j} M_j\right) \frameBasis_k
    \end{equation*}
    where
    \begin{equation*}
        \deflenx(\reflenx) \triangleq \int_0^{\reflenx} \|\bm{x}^{\prime}\| \, \mathrm{d}\reflenx
    \end{equation*}
    $$
    \frac{\mathrm{d} M_{\mathrm{a}}}{\mathrm{d} \deflenx}=\check{M}_{\mathrm{a}}=-d_{\mathrm{a}}^k\left(e_{k p q} t^p S^q+L_k\right)+d_{\mathrm{a}}^m w_m^k M_k
    $$
    
\iffalse
\item 
$$
\bm{\kappa}/\bm{K}? = \frac{1}{2}\begin{pmatrix}
\left. \frameBasis_2 \cdot \frameBasis_1^{\prime}-\frameBasis_1 \cdot \frameBasis_2^{\prime}-\FrameBasis_2 \cdot \FrameBasis_1^{\prime}+\FrameBasis_1 \cdot \FrameBasis_2^{\prime}\right. \\
\left. \frameBasis \cdot \frameBasis_2^{\prime}-\frameBasis_2 \cdot \frameBasis^{\prime}-\FrameBasis \cdot \FrameBasis_2^{\prime}+\FrameBasis_2 \cdot D^{\prime}\right. \\
\left. \frameBasis_1 \cdot \frameBasis^{\prime}-\frameBasis \cdot \frameBasis_1^{\prime}-\FrameBasis_1 \cdot \FrameBasis^{\prime}+\FrameBasis \cdot \FrameBasis_1^{\prime}\right. \\
\end{pmatrix}
$$
\fi 
...
\begin{ambox}[\gls{frame:exact} \gls{frame:shear} Frame (IFK)]{box:exact-shear-ifk}

\paragraph{Kinematics}
$$
\bm{K} = (\operatorname{dExp}\bm{\theta}) \bm{\theta}^{\prime} - (\operatorname{dExp}\bm{\theta}_0) \bm{\theta}_0^{\prime}
$$

\paragraph{Variations}
$$
\bm{A} = \left[\begin{array}{ccc}
\mathbf{1} & \mathbf{0} & \bm{x}^{\prime\,\times} \mathbf{T} \\ 
\mathbf{0} & \mathbf{T} & \mathbf{R}+\bm{K}^{\varphi} \mathbf{T} \end{array}\right]
$$
