...
\]
so that $\operatorname{A}[\bm{e}](\bm r)$ is the compatible pointwise strain vector at $\bm r\in\Section$.
...
Let $\mathcal{E}$ denote the space of admissible rod strain fields $\bm e(x)$ and define the image space
\[
\operatorname{B}^*[\mathcal{E}]
\;\coloneq\;
\left\{
...
\[
\tilde{\mathscr{E}} \cap \operatorname{B}^*[\mathcal{E}] = \{0\},
\]
...
Let $L \coloneq L_2(\Section;\mathbb{R}^3)$ denote the ambient space of stress vectors over the section, and let $\StressSpaceH\subset L$ denote the admissible stress space induced by the Saint-Venant restriction \eqref{eq:saint-venant-stress} together with the chosen material model and local stress constraint enforcement. The ideal consistency requirement is that the enhanced strain space is $L_2$-orthogonal to $\StressSpaceH$. In the present setting this is enforced by stationarity of the sectional internal work with respect to variations of the local parameters $\bm{\EnhanParam}$.
...
The enhanced parameters are determined by sectional stationarity of the internal work with respect to variations \(\delta\bm{\EnhanParam}\). Using the constrained stress response \(\StressVector(\cdot)\), this yields the residual \eqref{eq:def-enhan-residual}. 
When a strain-energy potential exists, the stationarity condition preserves the variational structure and leads to a consistent condensed tangent.
...
Condition (ii) is an ideal selection principle for the enhanced parameters $\bm{\EnhanParam}$. It determines $\bm{\EnhanParam}$ by imposing section-wise stationarity with respect to variations $\delta\bm{\EnhanParam}$, which enforces $L_2$ orthogonality of the enhanced strain space to the admissible stress space. When the material response admits a potential, this identification preserves the variational structure through the sectional condensation because $\bm{\EnhanParam}$ is obtained from the same stationarity principle. 
In contrast, stability is required for a well-posed split. Condition (ii) may be relaxed while still reproducing Saint-Venant fields exactly in the cases examined later, but the resulting beam model then lacks a variational interpretation.

\subsubsection{Implementation}

For a given beam strain state $\bm e(x)$, the enhanced parameters $\bm{\EnhanParam}(x)$ are obtained locally by solving \eqref{eq:def-enhan-residual} at the section. This local solve is performed concurrently with the material-point condensation implied by \eqref{eq:saint-venant-stress}: at each section quadrature point one evaluates $\bar{\bm\varepsilon}=\operatorname{A}[\bm e]+\EnhanShapes\bm{\EnhanParam}$, computes the constrained stress vector $\StressVector(\bar{\bm\varepsilon})$, and assembles $\residn(\bm{\EnhanParam})$. 
...
\end{align}
\end{subequations}
and substitution gives the condensed section tangent:
...
The controlling requirements are instead the identifiability condition (i) and the section-wise stationarity condition (ii), both stated directly in terms of the cross-sectional constitutive update.
