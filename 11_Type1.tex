...
% This is separated out of the equilibrium equation \cref{eq:finite-equilibrium}, so that 
...
\begin{remark}
For later use, we record the linearization of \eqref{eq:finite-equilibrium} with respect to the configuration variables.
Neglecting the body loads and the generalized warping load (for notational simplicity) one obtains
\[
  -\mathbb{B}\,\bm{\sigma} + \bm{f}
  =
  \left\{
  \begin{aligned}
    -\mathbb{B}_{\mathrm{p}}\,\bm{\sigma} + \bm{f} &= \mathbf{0}, \\
    \bm{w}' - \bm{v} &= \mathbf{0},
  \end{aligned}
  \right.
\]
...
\[
  ...
  \end{array}\right].
\]
Here $\mathbb{B}_{\mathrm{p}}$ denotes the restriction of $\mathbb{B}$ to the primary resultants $(\bm{n},\bm{m})$.
\end{remark}
...
\end{equation*}
This is separated out of the equilibrium equation \cref{eq:finite-equilibrium}, so that all assumptions on and simplifications to the primary kinematic variables \(\TraceLocation\) and \(\FrameRotation\) are contained within a \emph{primary} equilibrium operator \(\EquilibriumDerivative\). 
