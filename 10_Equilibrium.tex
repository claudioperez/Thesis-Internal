...
\begin{Aside}
Consider warping modes whose derivatives are constrained to the primary strains through \(\mathbf{L}_{ww}\bm{\alpha}' = \mathbf{L}_{\rm wp}\bm{e}_{\mathrm{p}}\). 
These arise naturally in theories such as Vlasov's non-uniform torsion where the warping amplitude is proportional to the twist rate.
\end{Aside}

It is common to assume that the amplitudes of certain warping modes are proportional to a component of one of the primary strain variables \(\bm{e}_{\mathrm{p}} = (\bm{\gamma},\,\bm{\kappa})\). For example, torsional warping is often assumed to be proportional to the rate of twist \(\varkappa \triangleq \bm{\kappa}\cdot \FrameBasis\).
...
Using the constraints \cref{eq:linear-warp-constraints}, the full strain vector under only algebraic constraints (\(\mathbf{L}_{\rm wp} = \mathbf{0}\) can be expressed through the transformation:
...
Using the decomposition \cref{eq:decompose-alpha} and the constraint equations, the test functions \(\delta\bm{\alpha}\) can be expressed in terms of the reduced variables. 
...
For linear constraints on warping with the form of \cref{eq:linear-warp-constraints}, the Type 1 constitutive response is transformed through:
\[
...
\begin{remark}
The treatment of the $\mathbf{L}_{\rm vp}^{\top}\bm{w}'$ term in \cref{eq:type2-equilibrium} reveals a fundamental distinction between section-level and element-level operations. The constitutive evaluation at a section operates pointwise and cannot access spatial derivatives. Therefore:
\begin{itemize}
...
Under conditions \cref{eq:type3-conditions}, the constitutive response and equilibrium simplify to:
\begin{equation*}
...
\end{ambox}
 ...
 \begin{Quote}
One major implication with Type 2 and 3 is that $\bar{\bm s}$ is not exactly the work-conjugate resultant to $\bar{\bm e}$. 
In Type 2 this may still furnish a symmetric \emph{global} tangent stiffness (linearizatino of the galerkin residual with respect to the variations of the configuration variables $(u,\Lambda,\alpha)$), because the element provides the $L_{vp} w'$ part (THIS IS A CONJECTURE THAT WE NEED TO VERIFY), but the section-level tangent certainly need not be symmetric (ie, $d\bar{s}/d\bar{e}$). 
In Type 3, any asymmetry in the section constitution $d\bar{s}/d\bar{e}$ goes right through to the global tangent.
\end{Quote}
...
A library of finite elements typically contains ``Type 1/2", and ``Type 3" finite elements. 
