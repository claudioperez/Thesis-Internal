...
Outline:
\begin{itemize}
    \item \cref{sec:nonuniform-prelim}: preliminaries and notation.
    \item \cref{sec:nonuniform-residual}: equilibrium violation.
    \item \cref{sec:nonuniform-enhanced}: construction of the minimal compatible correction space with one free parameter.
    \item \cref{sec:nonuniform-canonical}: canonical one-dimensional subspace.
...
The formulation by \cite{armero2021modeling} is obtained by 
setting \(\EnhanConstFA\) to ensure \(\chi_c\) is \(G\)-orthogonal
to \(\WarpFA\):
