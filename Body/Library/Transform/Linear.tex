...
%
\begin{equation}
\bar{\bm{x}}_I = \mathbf{0} = \bm{R}^{\mathrm{t}}\bm{X}_I + \bm{R}^{\mathrm{t}}(\bm{u}_I - \bm{c})
\quad\implies\quad 
...
\(\bm{R}(0)=\mathbf{1}\)) that \emph{maps the original unit direction}
...
\]
%
If \(\Delta \bm{X}\) and \(\Delta \bm{x}\) are not collinear, one
...
\(\theta^2(\varepsilon)/\varepsilon\to 0\). 
If \(\theta(\varepsilon)\)
