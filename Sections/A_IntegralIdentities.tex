...
\]
and using \cref{eq:gauss-div-thm} (Stoke's Theorem).
\end{proof}
...
The solution \(\WarpSV\) to the boundary value problem \cref{eq:warp-sv-bvp} is the Riesz vector:
\begin{equation}
...
&= \dd \phi \wedge \star (\nabla \WarpFA)^{\flat}  & \qquad \text{\cref{eq:hodge-zero}}\\
&= \dd \phi \wedge \star     \dd \WarpFA &  \qquad \text{\cref{eq:ident-ext-d}} \\
&= \left<\dd \WarpFA,\, \dd \phi \right> \volG & \qquad \text{\cref{eq:hodge-defn}} \\
\end{array}
\]
Thus (\(\dagger\)) is exactly an integration (by parts) of the product rule for \cref{eq:def-lie-form}
\[
\mathcal{L}_{(\WarpSV \nabla \WarpFA)} \volG = \WarpSV \mathcal{L}_{\nabla \WarpFA}\volG + \dd \WarpSV \wedge \interior{\nabla \WarpFA}{\volG}
\]
where by the divergence theorem (\cref{eq:gauss-div-thm})
\[
...
\begin{proposition}[Relocation]
    ...
    If \(\varphi_a\) is the solution to \cref{eq:torsion-neumann} with \(\bm{\pi} = \bm{\SectionLocation}_a\), then 
    \[
...
\end{proposition}
\begin{proof}
Our objective is to show that the function \(\varphi_b\) (i) is harmonic, (ii) satisfies the the boundary condition \cref{eq:torsion-neumann}$_2$, and (iii) satisfies the normalizing condition.
\begin{equation*}
