...
\begin{Quote}
The exposition follows the approach of \cite{iesan1986saintvenants}, whereby the Saint Venant class is decomposed into two problems: that of torsion-bending-elongation, and that of flexure.
...
The Saint~Venant problems concern a prismatic, homogeneous, and isotropic linearly elastic body 
\(
...
Given a resultant axial force \(\AxialRxn\) and couple \(\MomentRxn:=\TwistRxn\FrameBasis + \bar{M}_{\alpha}\,\FrameBasis_{\alpha}\) on \(\Section_{x_0}\),
...
In the recovered field, \((\FrameBasis_\alpha\otimes\FrameBasis_\beta)\,\StressTensor=0\) and  \eqref{eq:saint-venant-stress} holds.
...
\label{eq:iesan-2-22}
\end{equation}
where \(\Jsv\) is the torsion constant defined by \eqref{eq:def-j-origin}, and the \emph{shear point} \(\CenterShearLocation\in\Section\) is defined by:
\begin{equation}
