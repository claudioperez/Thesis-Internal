\subsection{Shear (Energetic) Trace (0)}%\label{sec:trace-energy}
...
\begin{equation*}
\delta\bm{e}\cdot \bm{s} = \int_\Section \delta\bm{\varepsilon} : \bm{\sigma} \, \mathrm{d}A
\end{equation*}
for all variations within the Saint~Venant class.

\paragraph{Local parameters and strain energy.}
...
= \frac{1}{2}\hat{\bm{e}}^\top \bar{\mathbf{C}} \hat{\bm{e}},
\end{equation}
...
0 & G\mathbf{h}_{\varphi\psi}^\top & G\Jo & \mathbf{0}^\top \\
...
\begin{align}
\mathbf{H}_{\psi\psi} &:= \int_\Section \mathbf{B}_{(b)}^\top \mathbf{B}_{(b)} \, \mathrm{d}A, \\
...
$\bm{s} = -\mathbf{R}(x)\,\bm{p}$, where $\mathbf{R}(x) = \mathbf{R}_0 + x\mathbf{R}_1$. 
...
0 & \mathbf{0}^\top & G\Jo & \mathbf{0}^\top \\
...
\paragraph{Energy conjugacy condition.}
...
\begin{align}
x^0: \quad & -\mathbf{X}^\top \mathbf{R}_0 \mathbf{L}_0^{-1} = \bar{\mathbf{C}}, 
...
From \eqref{eq:x0-condition}:
\begin{equation}
\mathbf{X}^{-1} = -\bar{\mathbf{C}}^{-1} \mathbf{L}_0^{-\top} \mathbf{R}_0^\top.
\label{eq:X-inv-formula}
\end{equation}

For \eqref{eq:x1-condition} to be consistent with a constant $\mathbf{X}$, 
we require the \emph{consistency condition}:
\begin{equation}
\mathbf{R}_0^{-1} \mathbf{R}_1 = \mathbf{L}_0^{-1} \mathbf{L}_1.
\label{eq:consistency-condition}
\end{equation}

\paragraph{Verification of consistency.}
...
so $\mathbf{R}_0^{-1}\mathbf{R}_1$ acts only on the $\bm{b}_{\Section}$ columns. 
...
\begin{equation*}
\mathbf{R}_0^{-1}\mathbf{R}_1 = \begin{pmatrix}
0 & \mathbf{0}^\top & 0 & -\CentroidLocation^\top \\
\mathbf{0} & \mathbf{0} & \mathbf{0} & \mathbf{0} \\
0 & \mathbf{0}^\top & 0 & \mathbf{0}^\top \\
\mathbf{0} & \mathbf{0} & \mathbf{0} & \oneS
...
\paragraph{Strain-Amplitude Matrix.}
...
The matrix $\mathbf{X}^{-1}$ converts beam strains 
$\bm{e} = (\epsilon, \bm{\gamma}_\perp, \varkappa, \bm{\kappa}_\perp)$ 
to local parameters $\hat{\bm{e}} = (\epsilon_0, \bm{\beta}, \hat{\varkappa}, \bm{\epsilon}_1)$. 
Several features are noteworthy:

\begin{itemize}
\item The $(1,1)$ entry is unity: axial strain equals the local axial parameter directly.

\item The $(4,4)$ block is $-\FrameBasis^\times$: curvature maps to the 
axial strain gradient with a sign consistent with the plane-section property.

\item The $(2,2)$ and $(3,2)$ blocks couple shear strain and twist to 
the bending stiffness $\IntRGoRG$, reflecting the energy-conjugate definition.

\item The $(2,3)$ and $(3,3)$ entries involve the shear center 
$\CenterShearLocation$ and the coupling vector $\bm{\eta}$, encoding 
the torsion-shear interaction.
\end{itemize}
...
$\mathbf{H}_{\psi\psi}$ & $\mathrm{L}^4$ & Flexural shear stiffness, 
    $\int_\Section \bm{\tau}_\psi^\top \bm{\tau}_\psi \, \mathrm{d}A$ \\
$\mathbf{h}_{\varphi\psi}$ & $\mathrm{L}^4$ & Shear-torsion coupling, 
    $\int_\Section \bm{\tau}_\psi^\top \bm{\tau}_\varphi \, \mathrm{d}A$ \\
$\tilde{\Jo}$ & $\mathrm{L}^4$ & Reduced torsion constant, 
    $\Jo - \mathbf{h}_{\varphi\psi}^\top \mathbf{H}_{\psi\psi}^{-1} \mathbf{h}_{\varphi\psi}$ \\
$\bm{\eta}$ & --- & Shear-torsion coupling ratio, 
    $\mathbf{H}_{\psi\psi}^{-1} \mathbf{h}_{\varphi\psi}$ \\[6pt]
...
$\mathbf{X}^{-1}$ & mixed & Beam strain to local parameter map \\
