...
%
The cantilever beam 
in Figure~\ref{fig:cantilever-1001} is subjected to \emph{either} a point moment \(\bm{M}\) 
or a point force \(F \, \mathbf{E}_3\) at its free end \(\xi=L\).
%
The centerline of the reference configuration 
is given by \(\bm{x}_0(\xi) = \xi\,\mathbf{E}_1\).
...
%
\subsubsection{End Moment (1001)}\label{sec:frame-1001}
%
%
...
\end{itemize}
%
In this case, the moment loading can be treated identically for all analyses by simply scaling a constant \emph{reference} force vector for the node at \(\xi=L\):
%
\begin{equation}
...
%
%
\subsubsection{End Force}\label{sec:frame-1002}
% \begin{figure}
% \centering
