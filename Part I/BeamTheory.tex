...
A major outcome of this chapter is a foundation for implementing constitutive (section) models that accommodate a wide variety of frame formulations.
...
\end{equation*}
This can be separated out of the total equilibrium operator \(\operatorname{B}\) of \eqref{eq:finite-equilibrium}, so that all assumptions on and simplifications to the primary kinematic variables \(\FramePosition\) and \(\FrameRotation\) are contained within a \emph{primary} equilibrium operator \(\EquilibriumDerivative\). Further investigation of \(\EquilibriumDerivative\) is defered to \cref{ch:frame-hierarchy}.
...
Recall the Galerkin projection \(\mathcal{G}_{\chi}[\bm{\eta}]\) defined by \eqref{eq:galerkin}
\begin{equation*}
...
\fi 
%
Using \eqref{eq:vstrain} for \(\delta \bm{e}_{\mathrm{p}}\triangleq D\bm{e}\cdot \bm{\eta}\) and exploiting linearity of the inner product again furnishes
\begin{equation*}
