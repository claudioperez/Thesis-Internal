...
\eqref{eq:finite-plane-kinematics} implies that plane sections remain plane. 
...
\end{equation}
%
For a linear isotropic material with shear modulus \(G\), \eqref{eq:finite-plane-kinematics} leads to the resultant torque \(T\) and the shears \(V_y,\,V_z\) in terms of the twist rate \(\vartheta'\) and the shear deformations \(\gamma_y,\, \gamma_z\) in the form: %\citep{hjelmstad2005fundamentals}:
\begin{equation*}
...
are introduced so as to match the exact solutions from the theory of elasticity in which the plane section hypothesis of \eqref{eq:finite-plane-kinematics} is violated. 
...
An alternative displacement field to \eqref{eq:finite-plane-kinematics} which naturally produces the resultants from \eqref{eq:correct-resultants} takes the form \citep{hjelmstad1987warping}:
\begin{equation}
...
Typically matching criteria are \parencite{nicholson1977timoshenko,steinboeck2013energyconsistent}
\begin{itemize}
