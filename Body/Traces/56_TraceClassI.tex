...
\mathbf{0} &
G\TraceShearArea &
G\TraceShearArea\TraceShearOrigin &
\mathbf{0}
\\ \hline &&& \\[-8pt] 
0 &
G\Jsv\,(\ShiftT)^\top
...
where:
\begin{align}
\TraceShearArea &:=G^{-1}E\IntRGoRG \ShiftB \\
\TraceShearOrigin &:=-\TraceFT \\
\TraceTwistOrigin &:=-\Jsv \TraceShearArea^{-1}\ShiftB\TraceTF
\end{align}
...
Block & Size &  Units & Description \\ 
\midrule
$\TraceShearArea$ & $\mathbb{R}^{2\times2}$  & \Length{2} %L   
& Shear--corrected area \\
...
\begin{align}
\ShiftB &=(E\IntRGoRG)^{-1}G\TraceShearArea \\
\ShiftK &=(E\IntRGoRG)^{-1}G\TraceShearArea\TraceShearOrigin \\
\ShiftT &=\Jsv^{-1}\TraceShearArea\TraceTwistOrigin \\
\ShiftScaleN &=\TraceNF\cdot\ShiftB\TraceFT \\
\ShiftScaleT &=1 - \TraceTF\cdot\ShiftK \\
\end{align}
