...
for element kinematics. The element kinematic formulations are
summarized in \cref{ch:frame-hierarchy} and are identified as
...
\texttt{mass\_type} & Flag & Optional flag variable specifying the mass
formulation. Options include \texttt{Rigid}, \texttt{Prism} and
\texttt{Taper} for non-prismatic elements. \\
\bottomrule
\end{longtable}
...
\paragraph{2. ForceFrame}
  The \texttt{Force} element is ideal for modeling inelasticity in
...
\paragraph{4. ExactFrame}
  The \texttt{Exact} element is a \(C^0\) displacement formulation of
...
\hypertarget{transformation-classes}{%
...
\inputdraft{Sections/A_Constants}
...
\texttt{Force} & Basic, Axial & Basic, Axial & Type I & Section & - \\
