...
\begin{comment}
%%%%%%%%%%%%%%%%%%%%%%% I commented this table unless we need to discuss it.
\begin{table}[hb]
% \begin{tabular}{@{}lllll@{}}
\begin{longtable}[]{@{}
  >{\raggedright\arraybackslash}p{(\columnwidth - 10\tabcolsep) * \real{0.2921}}
  >{\raggedright\arraybackslash}p{(\columnwidth - 10\tabcolsep) * \real{0.0225}}
  >{\raggedright\arraybackslash}p{(\columnwidth - 10\tabcolsep) * \real{0.0225}}
  >{\raggedright\arraybackslash}p{(\columnwidth - 10\tabcolsep) * \real{0.0225}}
  >{\raggedright\arraybackslash}p{(\columnwidth - 10\tabcolsep) * \real{0.6180}}@{}}
\toprule\noalign{}
...
\noalign{}
% \endlastfoot
\citet{bouc1967forced} & - & - & - &  \\
\citet{wen1976method} & - & - & - & \\
\citet{baber1981random} & - & a & - & Introduction of degradation to
\citet{wen1976method} \\
\citet{baber1985random} & - & a & a & Addition of pinching to
\citet{baber1981random} using series composition with a Gaussian. \\
\citet{baber1986modeling} & - & a & b & \\
\citet{sues1988systems} & - & b & - & New degradation function proposed for RC,
similar to Clough's \\
\citet{foliente1995hysteresis} & - & a & c & Proposed modification to
pinching function of \cite{baber1986modeling} \\
% \citet{reinhorn1995modeling} & & & & \\
% \citet{sivaselvan2000hysteretic} & & & & \\
% \citet{charalampakis2009bouc} & & & & \\
% \citet{kottari2014consistent} & & & & \\
% \citet{pelliciari2020degrading} & & & & \\
% \citet{liberatore2019enriched} & & & & \\
% \citet{amir2022statespace} & & & & \\
\bottomrule
\end{longtable}
\begin{tablenotes}
    \item[1] Slip models
    \item[2] Degradation models
\end{tablenotes}
\end{table}
...
% \tightlist
% \item
%   Asymmetric load cycles never form a stable closed loop; ie,
%   asymmetric load cycles always cause drift.
% \item
