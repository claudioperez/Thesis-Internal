\chapter{Integral Identities}\label{sec:integration}

\input{notation/integration}
\section{Preliminaries}

Box~\ref{box:recall} collects standard definitions and notation concerning integration on a Riemanian manifold \(\PlaneManifold\) with oriented volume form \(\mu\) \citep{marsden1994mathematical,abraham1988manifolds,shilov1996elementary,spivak1965calculus}.
...
\[
\begin{aligned}
    \varrho &\coloneq \interior{\nabla\WarpFA}{\volG}
            = G\,\starE \dd\WarpFA \;\in\Omega^{1}\PlaneManifold
    &\qquad
    \omega &\coloneq \ii_{\qvec}\dd A
            = \qvec^{\flat},
\end{aligned}
\]
with \(\qvec \coloneq \FrameBasis\times\bm{r}\). 
...
\end{equation}
where $\bm{\nu}$ is the unit vector normal to $\PlaneBoundary$ and the following identities are used:
$$
\interior{\bm{X}}{\mu} = \langle \bm{X},\, \bm{\nu}\rangle \underbrace{\interior{\bm{\nu}}{\mu}}_{\coloneq \eta_{\mu}}
$$
\end{enumerate}
}
\end{ambox}
...
$$
A\coloneq\int_{\mathscr D}\nabla u_0\otimes\nabla u_0\,dx,\quad
C\coloneq\int_{\mathscr D}\nabla w\otimes\nabla w\,dx\ \ (\text{sym.\ \& psd}),
$$

$$
S\coloneq\int_{\mathscr D}\!\big(\nabla u_0\otimes\nabla w+\nabla w\otimes\nabla u_0\big)\,dx\ \ (\text{sym.}).
$$
\end{enumerate}
...
Because $\operatorname{div}_{\mu}\bm{X}=0$ for any $h\in\mathcal F(\PlaneManifold)$ it follows
\[
...
\]
Integrating over $\PlaneManifold$ and applying the divergence theorem \eqref{eq:gauss-div-thm} yields
\[
...
\]
and using Stoke's Theorem \eqref{eq:gauss-div-thm}.
\end{proof}
...
$$
Thus it must be shown that the left hand side is equivalent to the (negative) $G$-energy inner product:
$$
\int_{\PlaneManifold} \phi\, (\operatorname{div}_{G} \nabla \WarpFA) \volG
=- \int_{\PlaneManifold} \nabla \WarpFA \cdot \nabla \phi \, \volG 
$$
...
\end{array}
\]
Thus (\(\dagger\)) is an integration by parts of the product rule for \eqref{eq:def-lie-form}
\[
...
% = \int_{\PlaneManifold} \WarpSV \, \frac{n_E}{n_G} \WarpSV  \mu
$$
It must be shown that the left hand side is equivalent to the (negative) $G$-energy inner product:
$$
\int_{\PlaneManifold} \phi\, (\operatorname{div}_{G} \nabla \WarpFA) \volG
=- \int_{\PlaneManifold} \nabla \WarpFA \cdot \nabla \phi \, \volG 
$$
...
Since $\delta \dd\WarpSV = \operatorname{div}_{G} (\nabla \WarpSV)$ it follows:
