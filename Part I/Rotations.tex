\hypertarget{sec:rotations}{%
\section{Rotations, Groups and Manifolds}\label{sec:rotations}}
...
\(\boldsymbol{\Lambda}\colon \mathbb{R}^3\rightarrow \mathbb{R}^3\), and
filtering out those which do not preserve the origin \(\boldsymbol{0}\),
the vector dot product \(\boldsymbol{a}\cdot \boldsymbol{b}\),
and the scalar triple product
\([\boldsymbol{a},\boldsymbol{b},\boldsymbol{c}] = \boldsymbol{a}\cdot\left(\boldsymbol{b}\times\boldsymbol{c}\right)\)
for vectors \(\boldsymbol{a},\boldsymbol{b}\) and \(\boldsymbol{c}\) in \(\mathbb{R}^3\). The
...
\mathrm{SO}(3) \triangleq \left\{\,
    \boldsymbol{\Lambda}\colon \mathbb{R}^3\rightarrow\mathbb{R}^3
~\left|~~
\begin{aligned}
\boldsymbol{\Lambda}\boldsymbol{a}\cdot\boldsymbol{\Lambda}\boldsymbol{b} &= \boldsymbol{a}\cdot\boldsymbol{b}\,
,  \\
[\boldsymbol{\Lambda a}, \boldsymbol{\Lambda b}, \boldsymbol{\Lambda c}]&=[\boldsymbol{a}, \boldsymbol{b}, \boldsymbol{c}]
,~\text{ and } ~\boldsymbol{\Lambda}\boldsymbol{0} = \boldsymbol{0}
\text{ for all }\boldsymbol{a},\boldsymbol{b},\boldsymbol{c}\in\mathbb{R}^3
\end{aligned}
\right.
\,\right\}
\label{eq:def-SO3}\end{equation}
It is indeed these properties alone that ensure any such map \(\boldsymbol{\Lambda}\) acts linearly on vectors\footnote{The linearity of $\mathrm{SO}(n)$ is more commonly specified directly by taking it as a subgroup of the general linear group, $\mathrm{GL}(n)$ (see e.g. \cite{artin1991algebra}).
...
\begin{aligned}
\boldsymbol{\Lambda}^{\mathrm{t}} \boldsymbol{\Lambda}&=\mathbf{1} \\
\det\boldsymbol{\Lambda} &= 1 .
\end{aligned}
\end{equation*}
...
rotation \(\boldsymbol{\varLambda}\) is stored in computer memory, and how
...
\(\varLambda_{ij} = \mathbf{e}_i\cdot\boldsymbol{\varLambda}\mathbf{e}_j\) on a basis
...
\(T_{\boldsymbol{R}}\mathrm{SO}(3)\) at an element \(\boldsymbol{R}\),
to be systematically related to the single tangent space
\(T_{\boldsymbol{1}}\mathrm{SO}(3)\) at the identity element \(\boldsymbol{1}\),
essentially representing the entire \emph{tangent bundle} \(\left\{T_{\scriptscriptstyle{\Lambda}}\,\mathrm{SO}(3)\mid \boldsymbol{\Lambda}\in\mathrm{SO}(3)\right\}\) %\FCF{???}
with a single vector space.
For this reason, the space \(T_{\boldsymbol{1}}\mathrm{SO}(3)\)
...
elements \(\boldsymbol{\Omega}\) of the Lie algebra \(\mathfrak{so}(3)\) behave like skew-symmetric linear operators. This immediately furnishes two convenient representations for these objects using either (1) vectors in \(\mathbb{R}^3\), or (2) the skew symmetric matrices in \(\mathbb{R}^{3 \times 3}\). 
Any vector \(\boldsymbol{\omega}\) in \(\mathbb{R}^3\) is associated
with a skew-symmetric matrix \(\boldsymbol{\Omega}\) using the \emph{hat} isomorphism
\((~\cdot~)^{\times}\). %\colon \mathbb{R}^3 \rightarrow \mathfrak{so}(3)\).
This is defined by the relation
\(\boldsymbol{\Omega}\boldsymbol{a} = \boldsymbol{\omega}^{\times} \boldsymbol{a}=\boldsymbol{\omega}\times \boldsymbol{a}\)
for any vector \(\boldsymbol{a}\) in \(\mathbb{R}^3\) and expressed in
coordinates as
\begin{equation}
\boldsymbol{\omega}^{\times}=\begin{pmatrix}
\omega_1 \\
\omega_2 \\
\omega_3
...
Similarly, given a skew-symmetric matrix \(\boldsymbol{\Omega}=\boldsymbol{\omega}^{\times}\),
the \emph{vee} isomorphism \((~\cdot~)^{\vee}\) is defined such that
\(\boldsymbol{\Omega}^{\vee}=\boldsymbol{\omega}\), which is expressed in
coordinates as: \begin{equation}
\boldsymbol{\Omega}^{\vee}= \begin{pmatrix}
0 & -\omega_3 & \omega_2 \\
\omega_3 & 0 & -\omega_1 \\
-\omega_2 & \omega_1 & 0
...
\begin{equation}
\left\langle\boldsymbol{\Omega}_{\boldsymbol{\Lambda}}, \boldsymbol{W}_{\boldsymbol{\Lambda}}\right\rangle
\triangleq \frac{1}{2} \operatorname{tr}\left[\boldsymbol{\Omega}_{\scriptscriptstyle{\Lambda}}^{\mathrm{t}} \boldsymbol{W}_{\scriptscriptstyle{\Lambda}}\right].
