...
%
  \item[Symmetric stiffness.] 
    ...
    (ii) the residual is represented on a different basis than the configuration increment.
    %When the former
  ...
  \item[Natural parameters.]
    When a configuration is modeled on a manifold, the nodal variables of the configuration may be very different from 
    those for 
    their increments. 
    ...
    the same size.
    Furthermore, this representation may be very different from that of the linearized theory, 
    which
...
%
  \item[Strain objectivity.]
    ...
    but consequently forfeits the symmetry of the tangent stiffness \citep{romero2004interpolation}.
%
...
%
  \item[Director constraints.]
    ...
    In this formulation, the constraint is effectively recovered through mesh refinement.
%    
\end{description}
%
...
Consequently, although a wealth of literature has addressed the issues above, certain combinations of these features have yet to be achieved simultaneously. 
...
% Despite taking a geometrically linear differential equation as a starting point, these formulations are known to successfully reproduce solutions of nonlinear boundary value problems and can be found in prominent commercial simulation software.
% However, because these methods take a discrete virtual work statement as their point of departure, it is not clear in what capacity these formulations approximate a governing differential equation.
% This creates the impression that elements derived in this manner belong to a class that is distinct from those arising from traditional finite element solutions of nonlinear differential equations. Even works like \cite{simo1987role} present these methods as if they fundamentally stem from a different set of governing equations.
