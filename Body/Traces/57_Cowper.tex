...
From
\(\bar{\bm{u}}'_\perp = (-\tfrac{1}{2}x^2 + \SectionAverage{\PlaneShearShape})\bm{b}_{\Section}\)
and \(\bm{u}'_\perp = \GaugeShearB - \tfrac{1}{2}x^2\bm{b}_{\Section}\):
\[
\GaugeShearB = \SectionAverage{\PlaneShearShape}\bm{b}_{\Section}
\]
...
Then:
\[
\bm{\gamma} = \GaugeShearB + \GaugeShearA = \SectionAverage{\PlaneShearShape}\bm{b}_{\Section} + \FrameBasis\times\bar{\Theta}[\FrameBasis\otimes\bm{\psi}]\bm{b}_{\Section} = (\ScaleShearGeomA)^{-1}\bm{b}_{\Section} = \bar{\bm{\gamma}}
\]

This confirms
\((\ScaleShearGeomA)^{-1} = \SectionAverage{\PlaneShearShape} + \FrameBasis\times\bar{\Theta}[\FrameBasis\otimes\bm{\psi}]\).

Define: \[
\ScaleShearGeomB := -\SectionAverage{\PlaneShearShape}\ScaleShearGeomA\]
...
i.e.,
\(-\SectionAverage{\PlaneShearShape} + \FrameBasis\times\bar{\Theta}[\FrameBasis\otimes\bm{\psi}]\ScaleShearGeomA = (\ScaleShearGeomA)^{-1}\)\ldots{}
...
\[
\GaugeShearB + \GaugeShearA = \SectionAverage{\PlaneShearShape}\bm{b}_{\Section} + \FrameBasis\times\bar{\Theta}[\FrameBasis\otimes\bm{\psi}]\bm{b}_{\Section} = (\ScaleShearGeomA)^{-1}\bm{b}_{\Section} = \bm{\gamma}
\]
...
\[
\bm{\gamma} = \left(\SectionAverage{\PlaneShearShape} + \FrameBasis\times\bar{\Theta}[\FrameBasis\otimes\bm{\psi}]\right) \bm{b}_{\Section}
...
\(\bm{\gamma}\):
\[
\GaugeShearB = \SectionAverage{\PlaneShearShape}\ScaleShearGeomA\bm{\gamma}, \qquad \GaugeShearA = \FrameBasis\times\bar{\Theta}[\FrameBasis\otimes\bm{\psi}]\ScaleShearGeomA\bm{\gamma}
\]
...
\[
\begin{aligned}
\GaugeShearB &= \SectionAverage{\PlaneShearShape}\ScaleShearGeomA\bm{\gamma}\\
...
\[
\begin{aligned}
\mathbf{L}_{\rm wn}^{(v)} &= -\SectionAverage{\PlaneShearShape}\ScaleShearGeomA \\
...
\[
\begin{aligned}
\mathbf{L}_{\rm wn}^{(v)} &= -\SectionAverage{\PlaneShearShape}\ScaleShearGeomA, && \mathbf{L}_{\rm vn}^{(v)} = 0 \\
\mathbf{L}_{\rm vn}^{(r)} &= -\FrameBasis\times\bar{\Theta}[\FrameBasis\otimes\bm{\psi}]\ScaleShearGeomA, 
...
\(\bar{\vartheta} = x\,\FrameBasis\cdot\bar{\Theta}[\PlaneShearShape]\bm{b}_{\Section}\).

Under pure flexure, matching would require
\(c_\vartheta = \FrameBasis\cdot\bar{\Theta}[\PlaneShearShape]\bm{b}_{\Section}\).
However, the Saint-Venant solution determines:
\[
\Jsv c_\vartheta = -\FrameBasis\cdot\int_{\Section} \bm{r}\times(\nabla\bm{\psi} + \PlaneShearShape)\bm{b}_{\Section} \dA
\]
...
where:
\[\bar{c}_{\vartheta} := \FrameBasis\cdot\bar{\Theta}[\PlaneShearShape]\bm{b}_{\Section} = \FrameBasis\cdot\bar{\bm{I}}^{-1}\int_{\Section} (\bm{r}-\bar{\bm{p}})\times\PlaneShearShape\,dA\,\bm{b}_{\Section}\]
...
where:
\[
\bar{\mathbf{g}} := \FrameBasis\cdot\bar{\bm{I}}^{-1} \int_{\Section} (\bm{r}-\bar{\bm{p}})\times\PlaneShearShape\,dA
\]
...
\(-\SectionAverage{\PlaneShearShape}\bar{\Xi}_\kappa\bm{\gamma}\) & \(\ShearForce\) &
\(0\) \\[0.3cm]
