...
\label{eq:2-15}
\end{equation}
where the axial rigidity \(\AxialRigidity\) and coupling vector \(\AxialCoupling\) are defined by:
\begin{equation}
\AxialRigidity \coloneq \int \volE 
\qquad\text{and}\qquad
\AxialCoupling \coloneq \int \bm{r}\volE 
...
\begin{equation}
\CoupleRigidity := \int \bm{r}\otimes\bm{r}\volE
\qquad\text{and}\qquad
\TwistRigidity:=\int_\Section \Big(r^2+(\FrameBasis\times\bm{r})\cdot\nabla\WarpTwistIesan\Big)\volG .
...
\left\{
\begin{aligned}
\Delta(\WarpShearShape \cdot\bm{b}_{\Section})&=\bm{f}_{\mathrm{I\!I}}\cdot\bm{b}_{\Section}
