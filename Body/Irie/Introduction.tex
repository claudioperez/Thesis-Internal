...
Providing programmatic access to large datasets through APIs has become essential across many fields of scientific research. Exposing SGMD through an API will streamline access for researchers, enabling reproducibility and facilitating real-time data analyses across diverse disciplines. Comparable platforms illustrate the value of public APIs:

\begin{itemize}
    \item \textbf{NASA's Earthdata Platform}: This platform offers continuous API access to climate and satellite data, allowing researchers to build predictive models and analyze environmental trends \parencite{NASA_Earthdata_2021}. Earthdata's infrastructure provides over 32 petabytes of data, enabling large-scale studies on climate impacts.
    \item \textbf{USGS National Map APIs}: The USGS provides geospatial data through APIs that are invaluable for resource management and environmental impact studies, supporting applications in natural resource planning \parencite{USGS_NationalMap_2021}.
    \item \textbf{NCBI E-utilities API}: Enabling access to biomedical data for genomic and bioinformatics research, the NCBI API handles millions of data requests daily, demonstrating the significance of such access in accelerating scientific discovery \parencite{Sayers_NCBI_2019}.
\end{itemize}
%
These examples underscore the benefits of APIs for fostering interconnected research ecosystems, where automated data workflows support reproducibility and large-scale analyses.
