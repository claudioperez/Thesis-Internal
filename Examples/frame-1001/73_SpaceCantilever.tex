...
%
The cantilever beam 
from Section~\ref{sec:frame-1002} is subjected to a \emph{combined} point moment \(\bm{M}\) 
and a point force \(F \, \mathbf{E}_3\) at its free end \(\reflenx=L\).
%
...
with the properties from Section~\ref{sec:frame-1001} for all cases
...
%
%
\paragraph{Simple Perturbation.}\label{sec:frame-1006}
Following \cite{ibrahimbegović1995computational}, the problem of plane flexure from \cref{sec:frame-1001} is now altered by introducing the point load
\(\bm{F} = 1/16 \, \mathbf{E}_3\) in addition to the moment \(\bm{M}\) so as to induce
a three-dimensional response. 
A uniform mesh of 10, 2-node \ExactFrame{I} elements is used, and the reference moment in \eqref{eq:fref} for \(\lambda = 1/8\) is applied in a single load step. 
...
%
\input{Examples/frame-1001/Tables/CantileverPerturbed01}

\paragraph{Consistent Perturbation.}
The problem is simulated again, but now the moment is consistently applied with a spatial orientation.
Formulations whose final residual moment vector is conjugate to the spatial variations \(\bm{u}_{\scriptscriptstyle{\Lambda}}\) of the rotation \(\FrameRotation\) do not need to be treated differently.
...
%
\input{Examples/frame-1001/Tables/CantileverPerturbed02}

\subsubsection{Oscillating Spiral \ShowLabel{1008}}\label{sec:frame-1008}

\begin{figure}[H]
\centerline{%
\begin{subfigure}[b]{0.48\textwidth}
\centering
\includegraphics[width=0.9\textwidth,keepaspectratio]{Examples/frame-1001/Figures/Figure_2a}
\end{subfigure}
\begin{subfigure}[b]{0.6\textwidth}
\centering
\includegraphics[width=0.9\textwidth,keepaspectratio]{Examples/frame-1001/Figures/Figure_2b}
\end{subfigure}
}
\caption{Deformed shape of the cantilever beam (Example~\ref{sec:frame-1008}) bent into a helical form at \(\lambda=1\) (left) alongside the relation between the tip displacement in the loading direction and the load factor $\lambda$ (right).}\label{fig:helix}
\end{figure}
%
To demonstrate the behavior of the
proposed formulations under large rotations, the reference moment value $M$ in \eqref{eq:fref} is now increased to \(\lambda=10\) with a large out-of-plane force of \(F=5 \lambda\).
The model discretization uses 100 2-node \ExactFrame{I} finite elements, and the loading is applied in 200 steps under load factor control.
