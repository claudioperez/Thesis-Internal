...
\begin{itemize}
    ...
    \item Modern analysis workflows often introduce design variables at a higher level. Typical examples are geometric dimensions, section generators, discretization rules, and load parametrizations.
...
parameterised by a vector of model inputs~$\corey$, the DDM
...
primal factorisation is available.  
The method is well established in
computational mechanics and forms the backbone of gradient-based
design optimisation, reliability analysis, and parameter identification
...
that \emph{determine} these core inputs through an arbitrarily complex
preprocessing chain.  Cross-section dimensions must be converted to
fiber discretisations; topology descriptors must be mapped to element
...
such transformation introduces a link in the chain rule that the DDM
alone cannot traverse.
...
linear algebra is possible in principle but is neither efficient nor
...
element procedures.  Python, for instance, does not provide sufficient
control over memory layout and allocation to support the kind of
low-level optimisation that production finite element codes require.
...
% Automatic differentiation (AD) frameworks---JAX, PyTorch, and their
% relatives---face the complementary limitation.  They can differentiate
% arbitrary host-language code with ease, but they are unaware of the
% implicit function defined by the finite element solve.  Treating the
% analysis as an opaque subroutine and differentiating through its
% internal linear algebra is possible in principle but is neither
% efficient nor numerically robust for large-scale structural models. 
% Furthermore, natural host environments like Python are generally not well suited for the efficient implementation of finite element procedures, as they do not provide sufficient control over memory. Even high level languages like Julia, which target scientific computing, [idk how to phrase it, but i'm just thinking about how in Julia things like memory management and the ability to distinguish runtime/compile time operations are not truly ``first class'' concepts like they are in C++]
...
\]
Assume that this problem is locally well posed so that a differentiable solution map exists,
\[
...
\]
denote the quantity of interest. The reduced response map is
\[
...
\bigr).
\]
This is the natural primal-tangent object in forward mode. It returns both the value and the Jacobian-vector product.
...
\subsection{Kernel level operators}
...
\subsection{Host level operators}
...
\CoreParameters=\ParameterMap(\HostParameters).
\]
If the quantity of interest is the full discrete state, then
\(\ResponseSpace=\StateSpace\) and
\(\ResponseMap_{\ModelFamily}=\StateMap_{\ModelFamily}\).

Its total derivative satisfies the chain rule
\begin{equation}
...
\Bigr],
\end{equation}
where \(\bm z = \ComposedResponse_{\ModelFamily}(\HostParameters)\).

The computation separates into two stages:
\begin{enumerate}
...
\end{equation}
If \(\ResponseMap_{\ModelFamily}=\StateMap_{\ModelFamily}\), then \(\bm z=\State\) and \(\dot{\bm z}=\dot{\State}\). Forward mode is natural when derivatives are required in only a small number of host directions.
...
\begin{remark}
If the kernel derivative is available in coordinates as the family
...
If reverse mode is later required, an adjoint state \(\AdjointState\) may be introduced at the kernel level for \(\ResponseMap_{\ModelFamily}\) or for \(\Objective\circ\ResponseMap_{\ModelFamily}\). This does not change the separation between the host and kernel layers.
...
using the DDM.  
From the perspective of the AD framework, $\Mmap$ is
...
\begin{itemize}
    \item Use \(\ModelFamily\) for the fixed discrete model family.
    \item Use \(\ParameterMap\) for the host-to-core parameterization.
    \item Use \(\StateMap_{\ModelFamily}\) for the implicit state solution map.
    \item Use \(\ResponseMap_{\ModelFamily}\) for the reduced response map.
    \item Use \(\Tan{f}\) for the primal-tangent extension of a map \(f\).
    \item Use \(\D f(x)[\dot x]\) for the Jacobian-vector product at a point.
\end{itemize}
