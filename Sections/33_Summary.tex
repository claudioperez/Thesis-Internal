...
%
The complete Saint-Venant solution consists of the particular solution \eqref{eq:iesan-general-solution} plus an arbitrary rigid body displacement:
...
where the rigid body field \(\hat{\bm{u}}_{\rm R}\) contains six arbitrary parameters. 
\begin{equation}
\begin{aligned}
...
\end{equation}
where
\(\bm{q}_u\in \mathbb{R}^3\)
represents translation and
\(\bm{q}_\theta\in \mathbb{R}^3\)
represents rotation. 
%
...
\begin{equation}
\snm(0) = \IesanStiff\bm{p}
\label{eq:snm0-in-p}
\end{equation}
where \(\bm{p}\in\mathbb{R}^6\) collects the six independent parameters of the Saint-Venant class: 
\begin{equation}
\bm{p} \coloneq (a_{\varepsilon},\, \bm{a}_{\Section},\, a_{\vartheta},\, \bm{b}_{\Section})
...
\begin{table}[H]
    ...
    Symbol & Units & & Description \\ \midrule
        \(a_{\varepsilon}\in\mathbb{R}\) & ---  & & Axial strain parameter \\
        ...
        \LightRule
        \(\bm{q}_u\in\mathbb{R}^3\) & & & Rigid translation at \(x=0\) \\
