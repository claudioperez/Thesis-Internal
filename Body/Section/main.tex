...
The construction is motivated by enhanced strain methods in finite element analysis, in which the compatible strain is augmented by additional modes that do not enter the admissible stress work.

For a prescribed set of beam strain measures, additional section strain modes are introduced to improve the representation of the underlying three-dimensional strain field. 
The associated mode amplitudes are determined locally at each cross section and are condensed out of the global system. 
...
\end{SaveEquation}
where \(\PlaneStrainTensor = \oneS \hat{\StrainTensor}\oneS\) so that \(\FrameBasis\otimes\FrameBasis\PlaneStrainTensor\,\FrameBasis\otimes\FrameBasis = \mathbf{0}\). The strain vector \(\hat{\StrainVector}\) is further decomposed:
\[
...
in-plane strain \(\PlaneStrainTensor\) contributes to the virtual work. 
...
The compatible in-plane 
contributions (such as \(\tfrac{1}{2}\nabla\AxialWarp\otimes\nabla\AxialWarp\)) 
and the condensed strains \(\PlaneStrainTensor_{\alpha\beta}\) are orthogonal to 
the residual and need not appear explicitly. 
...
% section coordinates \(\bm{r}\). 
The strain vector \(\hat{\StrainVector}\) may be truncated at various 
...
A direct way to improve the section kinematics is to enlarge the warping basis in \eqref{eq:finite-alignment-field}. This strategy is systematic, but it introduces additional one-dimensional warping amplitudes that must be discretized along \(\mathcal{C}\) and solved globally. In many applications, only a small number of such auxiliary fields is desirable, because much of the cross-sectional distortion is primarily a local response driven by the existing generalized strains \(\bm{e}\).

This motivates a sectional enhancement strategy intended to \emph{minimize} the number of auxiliary warping fields. 
The beam balance laws are retained, while the strain state entering the section constitutive update is enriched by additional cross-sectional modes. The corresponding mode amplitudes are solved locally at each section and condensed. 
...
Nevertheless, one may also prescribe $\bm{\EnhanParam}$ directly as a function of $\bm{e}$ (thus relaxing \eqref{eq:def-enhan-residual}); the resulting model need not be variational, but can still reproduce Saint-Venant solutions exactly in the cases considered later.
