...
\begin{Quote}
In this section we introduce material points that are not contained on the reference line of the beam model. 
The objective is to develop a family of constitutive relations for the material resultants \(\bm{s} = (n,m,w,v)\) and strains \(\bm{e} = (\gamma,\kappa,\alpha,\alpha')\) that represents these material points.

Everything is developed under the unconstrained Type 1 equilibrium equations.
\end{Quote}

\paragraph{Objectives}
\begin{itemize}
\item Internal Wagner nonlinearity
\item Arbitrary Geometry 
\begin{itemize}

    \item Torsion warping: For the linear shear-free case, a closed form solution is available which has been implemented by \cite{chaudhary1982generalized}

    \item Shear Warping : Shear deformability can play a significant role in the evaluation of the global stiffness of a structure and in the distribution of the internal forces among its constitutive members. This is the case, for instance, of the structural skeletons of tall buildings, subjected to horizontal forces, when the presence of concrete \emph{shear walls} is predominant.

    \item able to detect eventual elastic coupling between the warpings associated to torsion and shear forces
    
    \item General Geometry. C- and L-sections. Torsion-flexural coupling. \cite{kim2005exact,elfatmi2007nonuniform}, and \cite{du2020threedimensional}. 
...
\item Arbitrary Coordinates : All basic equations are formulated with respect to an arbitrary cartesian coordinate system which is not restricted to principal axes. 
Thus the origin of this system is not necessarily a special point like the centroid. 
...
Recall that up until this point we have regarded the beam as a purely one-dimensional body: the state of stress $\TraceTotalEnergyStress(\xi_m)$ and strain $\FrameStrain(\xi_m)$ at a point $\xi_m$ are mathematically related only to the material points immediately to their left $\xi_m-d\xi$ and right $\xi_m+d\xi$.
...
Recall that deformations of the reduced fields \(\bm{x}\) and \(\FrameRotation\) are measured objectively by the reduced strains \eqref{eq:def-gam-kap}:
...
\end{SaveEquation}
In terms of \(\bm{u}\triangleq\bm{x}- \reflenx\,\FrameBasis\) the linearized alignment strains are:
