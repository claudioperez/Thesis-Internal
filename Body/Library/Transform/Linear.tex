...
\end{equation}
%
To find \(\bar{\bm{C}}\) use \(\bar{\bm{c}}\) above in \cref{eq:iso-u}:
\[
\begin{aligned}
\bar{\bm{u}}(\xi)
...
\end{equation}
Then Rodrigues' formula (\cref{eq:ExpSO3}) furnishes 
\[
\bm{R} 
\;=\; 
\bm{1} 
...
\begin{enumerate}
    ...
    \end{pmatrix}
    \]
    with \(\bm{c}\). Translation increments from \cref{eq:iso-u}:
    \[
    \begin{aligned}
...
The exponential (\cref{eq:ExpSO3}) then furnishes for \(\varepsilon\neq 0\): 
\[
\bm{R}(\varepsilon)
\;=\;
\bm{1}
...
\(\varepsilon\neq 0\), and it indeed takes \(\Delta \bm{X}^*\) to
\(\Delta \bm{x}^*(\varepsilon)\). At \(\varepsilon=0\), \(\Delta \bm{x}^*(0)=\Delta \bm{X}^*\),
so \(\theta(0) = 0\) and
...
Note that \cref{eq:linear-Dxstar} implies 
\[
  \Delta \bm{X}^* \,\cdot\, \Delta \bm{x}^*(\varepsilon)
  \;=\;
  1
...
\[
  ...
  \bigr),
\] 
Therefore, to first order, \cref{eq:order-DXDx} implies
\[
  \theta(\varepsilon)
  \;=\;
...
\subsection{Linearization (2)}

%
Below is a ``difference‐quotient'' style derivation starting with the usual axis‐angle (Rodrigues)
definition of \(\bm{R}(\varepsilon)\) (which is ill‐defined at
\(\varepsilon=0\)) to obtain a well‐defined derivative \(R' (0)\). 
...
\item
  The leading term
  \(\sin \theta(\varepsilon) \,\AlignAxis(\varepsilon)^{\times}\)
  typically goes like
  \(\theta(\varepsilon)\, \AlignAxis(\varepsilon)\). 
  If \(\theta(\varepsilon)\) is small,
  \(\theta(\varepsilon)\sim \|\Delta \bm{X}^*\times \Delta \bm{x}^*\|\)
  is typically order \(\varepsilon\) or sometimes
  \(\varepsilon^2\), etc.

\item
  The second term \((1-\cos\theta)\,[\AlignAxis]_{\times}^2\) is
  likewise an indeterminate form.
\end{enumerate}

% \subsubsection{\texorpdfstring{The difference quotient
% \(\bigl(\bm{R}(\varepsilon)-1\bigr)/\varepsilon\)}{The difference quotient (R(e)-1)/e}}

We will do this by explicitly taking the difference quotient and letting \(\varepsilon \to 0\).
We want
%
\[
  \lim_{\varepsilon\to 0}
...
\]
%
If that limit exists, it \emph{defines} \(R'(0)\). 
To see the limit does exist, let us rewrite the first‐order part of the exponential map in
...
\[
  ...
  \;+\;
  \underbrace{
    \sin \theta(\varepsilon)\,[\AlignAxis(\varepsilon)]_{\times}
  }_{\displaystyle \text{(term A)}}
  \;+\;
  \underbrace{
    \bigl(1-\cos[\theta(\varepsilon)]\bigr)\,[\AlignAxis(\varepsilon)]_{\times}^2
  }_{\displaystyle \text{(term B)}}.
\] Hence 
\[
  \frac{\bm{R}(\varepsilon) - \bm{1}}{\varepsilon}
  \;=\;
...
\tightlist
\item
  \textbf{Term B} \(\to \mathbf{0}\) as \(\varepsilon\to 0\).\\
\item
  \textbf{Term A} \(\to \bigl[\boldsymbol{\omega}\bigr]_{\times}\), with
  \(\boldsymbol{\omega} = \tfrac{\Delta \bm{X}\times \Delta \bm{u}}{\|\Delta \bm{X}\|^2}\).
\end{enumerate}
%
and ultimately that the limit is exactly \(\boldsymbol{\omega}^{\times}\). 
We will proceed from the axis--angle formula for \(\bm{R}(\varepsilon)\) and do a \emph{difference‐quotient
limit} (i.e.~``differentiate first, \emph{then} evaluate at
\(\varepsilon=0\)'').

\paragraph{\texorpdfstring{Term B \(\to \mathbf{0}\)}{Term B}}

Recall that \(1 - \cos\theta = O(\theta^2)\). 
In the usual scenario
(where \(\Delta \bm{u}\) has some component \emph{perpendicular} to
\(\Delta \bm{X}\)), the angle \(\theta(\varepsilon)\) scales linearly
with \(\varepsilon\), so \(\theta^2(\varepsilon)/\varepsilon\) goes to
\(0\). Even in the degenerate case (purely parallel displacement, so
\(\theta\) is second order in \(\varepsilon\)),
\(\theta^2/\varepsilon\to0\) still holds. 
Therefore 
\[
  \frac{1-\cos[\theta(\varepsilon)]}{\varepsilon}
  \;=\;
  O\!\Bigl(\frac{\theta^2}{\varepsilon}\Bigr)
  \;\to\;
  0,
  \quad
  \text{as }\varepsilon\to 0.
\]
Hence \textbf{Term B} vanishes in the limit, regardless of direction
of \(\Delta \bm{u}\).

\paragraph{\texorpdfstring{Term A's limit =
\(\bigl[\boldsymbol{\omega}\bigr]_{\times}\)}{Term A's limit = \textbackslash bigl{[}\textbackslash boldsymbol\{\textbackslash omega\}\textbackslash bigr{]}\_\{\textbackslash times\}}}\label{term-as-limit-biglboldsymbolomegabigr_times}

We need
\[
  \lim_{\varepsilon\to 0}
  \frac{\sin \theta(\varepsilon)}{\varepsilon}
  \,\AlignAxis(\varepsilon)^{\times}.
\] 
For the angle between two unit vectors \(\Delta \bm{X}^*\) and
\(\Delta \bm{x}^*(\varepsilon)\), we know\\
\[
  \sin \theta(\varepsilon) 
  \;=\;
  \|\Delta \bm{X}^*\times \Delta \bm{x}^*(\varepsilon)\|.
...
\[
  ...
  \;\frac{
    \bigl[\Delta \bm{X}^*\times \Delta \bm{x}^*(\varepsilon)\bigr]^{\times}
  }{
    \|\Delta \bm{X}^*\times \Delta \bm{x}^*(\varepsilon)\|
  }.
\] 
But the second fraction is precisely a \emph{unit} cross‐product operator in the direction of   \(\Delta \bm{X}^*\times \Delta \bm{x}^*(\varepsilon)\). 
So overall, \emph{Term A} is 
\[
    \frac{1}{\varepsilon}
    \,\bigl[\Delta \bm{X}^*\times \Delta \bm{x}^*(\varepsilon)\bigr]_{\times}.
\]
%
Hence
%
\[
  ...
  \lim_{\varepsilon\to 0}
  \frac{
    [\Delta \bm{X}^*\times \Delta \bm{x}^*(\varepsilon)]_{\times}
  }{\varepsilon}
  \;=\;
  \Bigl[\,
    \lim_{\varepsilon\to 0}
...
\] So we just need \[
  \lim_{\varepsilon\to 0}
  \frac{\Delta \bm{X}^*\times \Delta \bm{x}^*(\varepsilon)}{\varepsilon}.
\]
%
From \cref{eq:linear-Dxstar} 
\[
  \Delta \bm{x}^*(\varepsilon)
  \;=\;
  \frac{\Delta \bm{x}(\varepsilon)}{\|\Delta \bm{x}(\varepsilon)\|}
  \;=\;
  \frac{\Delta \bm{X} + \varepsilon\,\Delta \bm{u}}{\|\Delta \bm{X}\| + O(\varepsilon)}
  \;=\;
  \Delta \bm{X}^*
  \;+\;
  \varepsilon
  \,\frac{\Delta \bm{u} - (\Delta \bm{X}^*\cdot \Delta \bm{u})\,\Delta \bm{X}^*}{\|\Delta \bm{X}\|}
  \;+\;
  O(\varepsilon^2).
\] (We subtract the parallel component from \(\Delta \bm{u}\) so that
dividing by \(\|\Delta \bm{X}\| + O(\varepsilon)\) is accounted for
consistently to first order.) 
Therefore,
...
\[
  ...
  \Bigl(
    ...
    + O(\varepsilon^2)
  \Bigr).
\] But \(\Delta \bm{X}^*\times\Delta \bm{X}^*=0\). Also
\(\Delta \bm{X}^*\times\bigl[(\Delta \bm{X}^*\cdot \Delta \bm{u})\,\Delta \bm{X}^*\bigr]=\mathbf{0}\).
So to first order,

\[
  \Delta \bm{X}^*\times \Delta \bm{x}^*(\varepsilon)
  \;=\;
  \varepsilon\,\frac{1}{\|\Delta \bm{X}\|}
  \,\Bigl(\Delta \bm{X}^*\times \Delta \bm{u}\Bigr)
  \;+\;
  O(\varepsilon^2).
...
\[
  ...
  \frac{\Delta \bm{X}^*\times \Delta \bm{x}^*(\varepsilon)}{\varepsilon}
  \;=\;
  \frac{1}{\|\Delta \bm{X}\|}\,\Bigl(\Delta \bm{X}^*\times \Delta \bm{u}\Bigr)
  \;=\;
  \frac{1}{\|\Delta \bm{X}\|^2}\,\Bigl(\Delta \bm{X} \times \Delta \bm{u}\Bigr).
\] 
Therefore
\[
  \lim_{\varepsilon\to 0}
  \frac{\sin \theta(\varepsilon)}{\varepsilon}\, \AlignAxis(\varepsilon)^{\times}
  \;=\;
  \left[
    \frac{\Delta \bm{X}\times \Delta \bm{u}}{\|\Delta \bm{X}\|^2}
  \right]^{\times}.
\]
Recalling that the \((1-\cos\theta)/\varepsilon\) term goes to \(0\), we conclude
\begin{equation*}
\boxed{
...
derivative is zero. Also \(\bm{R}(0)=\bm{1}\). Thus \[
  ...
  \left.\frac{\mathrm{d}}{\mathrm{d}\varepsilon}\Delta \bm{x}^*(\varepsilon)\right|_{\varepsilon=0}.
\] 
Using \cref{eq:linear-Dxstar} for 
...
\(\boldsymbol{\omega}\) such that for \emph{all} \(\mathbf{x}\), \[
...
\[
  \boldsymbol{\omega}\times \Delta \bm{X}^*
  \;=\;
  \frac{1}{\|\Delta \bm{X}\|}
  \Bigl[
    \Delta \bm{u}
...
\] 
One can check that
\(\boldsymbol{\omega} = \tfrac{1}{\|\Delta \bm{X}\|}\,\bigl(\Delta \bm{X}^*\times \Delta \bm{u}\bigr)\)
...
\subsection{Linearization (4)}

%
Our goal is to linearize \(\bm{R}(\varepsilon)\) about \(\varepsilon=0\); i.e.~find 
\[
  \bm{R}(\varepsilon)
  \;=\;
  \bm{1}
  \;+\;
  \varepsilon\,\boldsymbol{\Omega}
  \;+\;
  O(\varepsilon^2),
\] where \(\boldsymbol{\Omega}\) is a skew-symmetric matrix.

\subsubsection{\texorpdfstring{Rotation \(\bm{R}(\varepsilon)\)}{Rotation R(e)}}
...
\] Suppose that \[
  ...
  O(\varepsilon^2),
\] 
where \(\boldsymbol{\Omega}\) is skew-symmetric. 
Then
\[
  \bm{R}\,\Delta \bm{X}^\ast
  \;=\;
  \Delta \bm{X}^\ast
  \;+\;
  \varepsilon\,\boldsymbol{\Omega}\,\Delta \bm{X}^\ast
  \;+\;
  O(\varepsilon^2).
\] 
Matching this with
\(\Delta \bm{x}^\ast(\varepsilon)\approx \Delta \bm{X}^\ast + \Delta(\varepsilon)\),
we see that
\[
  \varepsilon\,\boldsymbol{\Omega}\,\Delta \bm{X}^\ast
  \;=\;
  \Delta(\varepsilon)
\]
and with \cref{eq:delta-basis}

\[
  \varepsilon\,\boldsymbol{\omega} {\times}\,\Delta \bm{X}^\ast
  \;=\;
  \Delta(\varepsilon)
  \;=\;
  \varepsilon
...
\]
Canceling \(\varepsilon\) on both sides gives
\[
  \boldsymbol{\omega} \;\times\; \Delta \bm{X}^\ast
  \;=\;
  \frac{1}{\|\Delta \bm{X}\|}
  \Bigl[
     \Delta \bm{u}
  ...
  \Bigr].
\] 
A standard vector identity (the ``triple cross product'') shows that
choosing \[
  \boldsymbol{\omega}
  \;=\;
  \frac{1}{\|\Delta \bm{X}\|}
  \bigl(\Delta \bm{X}^\ast \times \Delta \bm{u}\bigr)
...
\] 
Indeed, using
\(\Delta \bm{X}^\ast \triangleq \tfrac{\Delta \bm{X}}{\|\Delta \bm{X}\|}\) we may
rewrite
\[\boldsymbol{\omega} = \tfrac{1}{\|\Delta \bm{X}\|^2}\,\bigl(\Delta \bm{X} \times \Delta \bm{u}\bigr)\]
%
Altogether, the linearized rotation is:
...
\begin{equation}
  ...
  \bm{1}
  \;+\;
  \varepsilon\,\bigl[\boldsymbol{\omega}\bigr]_{\times}
  \;+\;
  O(\varepsilon^2),
  \quad
  \text{with}
  ...
  \boldsymbol{\omega}
  \;=\;
  \frac{\Delta \bm{X}^\ast \times \Delta \bm{u}}{\|\Delta \bm{X} \|}
  \;=\;
  \frac{\Delta \bm{X} \times \Delta \bm{u}}{\|\Delta \bm{X}\|^2}.
  }
\end{equation}
...
\item
  Dividing by its own norm yields a \emph{unit vector} that converges to
  the direction of \(\Delta \bm{X}^*\times \Delta \bm{u}\) (if that is
  nonzero).
\end{itemize}
%
...
\(\Delta \bm{X}^*\times(\Delta \bm{X}^*+\dots)\). Collecting these
expansions yields precisely
\[
\boldsymbol{\omega}=\tfrac{\Delta \bm{X}\times \Delta \bm{u}}{\|\Delta \bm{X}\|^2}
\].
