...
The governing equations for a family of Type~II beam theories is recalled in \cref{sec:genwarp}, through which the warping-free Type~I family may be regarded as a subset. 
The objective is to obtain a model on a curve \(\mathcal{C}\) that represents finite deformations of a three-dimensional body \(\Body\) exhibiting small to moderate strains.
...
\Cref{sec:enhan-section} introduces a family of strain-enhanced section constitutive models in the setting of \cref{sec:genwarp}. 
...
\end{SaveEquation}
where \(\PlaneStrainTensor = \oneS \hat{\StrainTensor}\oneS\) so that \(\FrameBasis\otimes\FrameBasis\PlaneStrainTensor\,\FrameBasis\otimes\FrameBasis = \mathbf{0}\) and the strain vector \(\hat{\StrainVector}\in\mathbb{R}^3\) contains the axial \(\PointAxialStrain\in\mathbb{R}\) and shear \(\PointShearStrain\in\Section\) terms:
\[
...
In the present setting this is enforced by stationarity of the sectional internal work with respect to variations of the local parameters $\bm{\EnhanParam}$, which is equivalent to requiring:
\begin{equation}
    \residn(\bm{\EnhanParam}) = \mathbf{0}
    \label{eq:enhan-cii}
\end{equation}
where the residual \(\residn\) is:
...
\end{SaveEquation}
When a strain-energy potential exists, the stationarity condition \eqref{eq:enhan-cii} preserves the variational structure.


When \eqref{eq:enhan-cii} holds, virtual work contributions associated with variations $\delta\bm{\EnhanParam}$ vanish, so the beam-level balance laws remain those of \cref{sec:genwarp}; only the section constitutive mapping $\bm{e}\mapsto \bm{s}$ is modified through the enhanced stress response.
...
The condition \eqref{eq:enhan-cii} may be relaxed while still reproducing Saint-Venant fields exactly in the linear elastic regime, but the resulting beam model then lacks a variational interpretation. This is investigated further in \cref{sec:timo}. 
