...
\begin{equation}
\begin{aligned}
\big(\CoupleRigidity\,\bm{a}_{\Section}+a_{\varepsilon}\,EA\CentroidLocation\big)&=\MomentRxn\times\FrameBasis,\\
\big(\bm{a}_{\Section}\cdot EA\CentroidLocation+\AxialRigidity a_{\varepsilon}\big)&=-\AxialRxn,\\
...
\end{equation}
and the torsion constant \(\TwistRigidity\) and plane inertia tensor \(\CoupleRigidity\) are defined by:
\begin{equation}
\CoupleRigidity := \int E\bm{r}\otimes\bm{r}\dA
\qquad\text{and}\qquad
\TwistRigidity:=\int_\Section G\Big(r^2+(\FrameBasis\times\bm{r})\cdot\nabla\WarpTwistIesan\Big)\dA .
...
\begin{equation}
\begin{aligned}
\big(E\IntRoR\,\bm{b}_{\Section}+EA\,\CentroidLocation\,b_\varepsilon\big)&=-\ShearRxn,\\
...
\begin{equation}
\CenterShearLocation := \frac{1}{\TwistRigidity} \int_\Section G(\nabla\WarpShearShape{} + \PlaneShearShape^\top)\,\bm{r} \times \FrameBasis \dA.
\qquad\text{with}\qquad 
\PlaneShearShape(\PlanePosition)\coloneq -\,\nu\big(\operatorname{dev}\bm{r}\otimes\bm{r}-\bm{r}\otimes\CentroidLocation\big)
\label{eq:def-shear-center}
\end{equation}
and \(\TwistRigidity\) is the torsion constant defined by \eqref{eq:def-j-origin}, and \(\PlaneShape\) is the solution to ...
