...
\begin{Quote}
TODO: Need to justify dropping boundary terms.
\end{Quote}
...
In a later section we want to show that when this Type 3 condensed/wrapped section is used in a ``standard" 6-DOF shear deformable beam finite element (which expects only 6 resultants), it is possible to exactly model the complete Saint Venant Class, without any alterations to the element. Furthermore, we want that when the section exhibits material or geometric nonlinear response through the resultant mapping, the wrapper-type3 element combination remains a ``consistant" approximant to a meaningful governing equilibrium BVP. In the saint venant-class,  differential constraints only arise in a few fields which exhibit several simplifying features which I hope will allow us to sufficiently restrict the treatment of such constraints so as to eliminate all the messy implications they entail. 
\end{Quote}
...
\subparagraph{Earthquake Analysis}
\cite{ngoc2017damage}
