\section{The Energy-Conjugate Trace}
...
\label{eq:tau-phi} \\
\mathbf{B}_{(\WarpShearIesan)}(\bm{r}) &\coloneq (\nabla\WarpShearShape{})^{\top} + \PlaneShearShape(\bm{r}).
\label{eq:tau-psi}
\end{align}
\end{subequations}
...
\end{equation*}
where the Saint~Venant energy matrix \(\IesanEnergy\) is
\begin{equation}
\begin{aligned}
\IesanEnergy := \int_\Section \mathbf{S}_0^\top \mathbb{C}\, \mathbf{S}_0 \dA 
&=\begin{bmatrix}
\AxialRigidity & \AxialCoupling^\top & 0 & \mathbf{0} \\
\AxialCoupling & \CoupleRigidity % E\IntRoR 
& \mathbf{0} & \mathbf{0} \\
0 & \mathbf{0} & \TwistRigidity & (\TwistRigidity\CenterShearLocation + G\mathbf{h}_{\WarpTwistIesan})^\top \\
\mathbf{0} & \mathbf{0} & (\TwistRigidity\CenterShearLocation + G\mathbf{h}_{\WarpTwistIesan}) & G(\mathbf{H}_{\WarpShear} + \Jsv\CenterShearLocation\otimes\CenterShearLocation + \CenterShearLocation\otimes\mathbf{h}_{\WarpTwistIesan} + \mathbf{h}_{\WarpTwistIesan}\otimes\CenterShearLocation)
\end{bmatrix}
\end{aligned}
\label{eq:energy-matrix}
\end{equation}
%
and the following definitions have been introduced:
\begin{subequations}
\begin{align}
\mathbf{H}_{\WarpShear} &:= \int_\Section \mathbf{B}_{(b)}^\top \mathbf{B}_{(b)} \dA, 
...
\IesanEnergy^{-1}
&=\begin{bmatrix}
(\AxialRigidity)^{-1} + \CentroidLocation\cdot (\ShearRigidity)^{-1}\CentroidLocation 
& -\CentroidLocation^\top (\ShearRigidity)^{-1} 
& 0 
& \mathbf{0} \\[6pt]
...
where 
\begin{equation}
% \tilde{\Jsv} := \Jsv - \mathbf{h}_{\WarpTwistIesan}\cdot \mathbf{H}_{\WarpShear}^{-1} \mathbf{h}_{\WarpTwistIesan}, 
\tilde{\mathbf{H}}_{bb} := \mathbf{H}_{\WarpShear} - \frac{1}{\Jsv}\mathbf{h}_{\WarpTwistIesan}\otimes\mathbf{h}_{\WarpTwistIesan},
\qquad\text{and}\qquad
\TraceTF := \frac{1}{\Jsv}\bigl(\Jsv\CenterShearLocation + \mathbf{h}_{\WarpTwistIesan}\bigr)
\qquad 
% \bm{\eta} := \mathbf{H}_{\WarpShear}^{-1} \mathbf{h}_{\WarpTwistIesan}.
...
& 0 
& \ShiftNK
% -EA\Bigl(\dfrac{1}{EA} + \CentroidLocation\cdot (\ShearRigidity)^{-1}\CentroidLocation\Bigr)
% (\FrameBasis\times\CentroidLocation)^\top
% + \CentroidLocation^\top (\ShearRigidity)^{-1}(E\FrameBasis^\times\IntRoR)^\top
\\[8pt]
\mathbf{0} & \mathbf{0} & \mathbf{0} 
& \FrameBasis^{\times} 
\\ \hline && \\[-8pt]
...
\begin{equation}
\begin{aligned}
\ShiftB &= (G\tilde{\mathbf{H}}_{bb})^{-1}(\ShearRigidity)^\top &&\sim L^{-2} \\ 
\ShiftA &= \mathbf{0},\\
\TraceTF &= \frac{1}{\Jsv}(\Jsv \CenterShearLocation + \mathbf{h}_{\WarpTwistIesan}) &&\sim L \\
\TraceFT 
%&= G(\ShearRigidity)^{-1}(\Jsv \CenterShearLocation + \mathbf{h}_{\WarpTwistIesan}) \sim L \\
 &= \TwistRigidity (\ShearRigidity)^{-1}\TraceTF &&\sim L \\
\ShiftN &= \mathbf{0},\\[2pt]
\ShiftNK &= -\Bigl(1 + \AxialCoupling\cdot (\ShearRigidity)^{-1}\CentroidLocation\Bigr)
  (\FrameBasis\times\CentroidLocation)
  + \FrameBasis\times E\IntRoR\,(\ShearRigidity)^{-1}\CentroidLocation && \sim L \\
\ShiftF &= \mathbf{0},\\ 
\hline \\[-8pt]
\ShiftT 
%&= -\frac{1}{\Jsv}\,\ShiftB^\top\bigl(\Jsv\CenterShearLocation + \mathbf{h}_{\WarpTwistIesan}\bigr) \\
 ...
 %
\ShiftK &= -(G\tilde{\mathbf{H}}_{bb})^{-1}\bigl(\TwistRigidity\CenterShearLocation + G\mathbf{h}_{\WarpTwistIesan}\bigr) \\
\ShiftScaleT 
