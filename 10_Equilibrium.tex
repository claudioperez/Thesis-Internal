...
\begin{remark}
Here $\bar{\bm{\alpha}}$ partitions out the constrained amplitudes, and $\bm{\alpha}$ is the union of these constrained amplitudes with the unconstrained amplitudes. Thus, for example, given a two-mode warping model $\bm{\alpha} = (\alpha_1, \alpha_2)$, and one constraint, $\alpha_1 = \FrameBasis\cdot\bm{\kappa}$,
then \[
...
This motivates the Type 3 family, where the $\bm{w}'$ term is dropped entirely, allowing all operations to remain at the section level and enabling the use of standard 6-DOF beam elements without modification.
\end{remark}

\subsection{Type 3: Uniform Warping}

Type 3 imposes two assumptions on the constrained warping modes:
\begin{equation}
