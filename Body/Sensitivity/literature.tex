...
explores applications of \autodiff{} to fields like structural modeling
...
\subsection{Direct Differentiation Method (DDM) in
...
For example, \cite{haukaas2003finite} implemented a comprehensive DDM-based sensitivity module in the OpenSees platform, enabling accurate and efficient computation of
...
DDM yields sensitivities that are both accurate and
computationally efficient. 
\cite{scott2004response}, 
applied DDM to nonlinear beam-column elements and highlighted that it
offers a robust and efficient approach for response sensitivity analysis of yielding structures.
...
\subsection{Automatic Differentiation in Computational Mechanics}\label{automatic-differentiation-in-computational-mechanics}
...
\item
  ...
  equations in a natural way, and the \autodiff{} system generates the Jacobian.
\item
  \textbf{Differentiable FE Libraries:} 
  For example, \cite{liang2023pytorchfea} introduced
  ...
  Notably, PyTorch-FEA was shown to
  significantly speed up forward simulations (benchmarking
  ...
  neural nets. 
  The implication is that \autodiff{} allows tight integration
  ...
  JAX-FEM \citep{xue2023jaxfem}  is a differentiable 3D FE solver built on Google's JAX library \citep{frostig2018compiling}, allowing GPU-accelerated computations. 
...
\subsection{Hybrid Approaches}\label{hybrid-approaches-combining-ddm-and-ad}
...
strategy''}. 
In other words, one should \textbf{``select the
...
We aim to differentiate the FE \emph{element kernel}
