\section{Traces}\label{sec:trace}
...
There are infinitely many traces that may exactly represent a given particular solution \(\hat{\bm u}_{\rm P}\) to a Saint-Venant problem, and Each imparts a distinct physical meaning to boundary conditions like \(\bm{\theta}_{\mathcal{T}}(x_0) = \mathbf{0}\) in a finite element simulation with displacements differing by the rigid homogeneous solution \(\hat{\bm u}_{\rm R}\). 

In general a trace \(\mathcal{T}\) may be defined as a functional which operates on continuum fields, as for example the geometric representation \eqref{eq:def-section-avg-trace}. 
However, because we only require exact consistency under finite dimensional space of Saint~Venant solutions, we seek to identify traces by membership in the following equivalence class\footnote{Equivalence classes are expounded on, for example, by \cite{halmos1974finitedimensional}.}:

\begin{definition}[Saint~Venant Equivalence]
\label{def:sv-equivalence}
Two traces $\mathcal T_1,\mathcal T_2$ are \emph{Saint~Venant-equivalent} if
$\mathcal T_1[\hat{\bm u}]=\mathcal T_2[\hat{\bm u}]$ for all $\hat{\bm u}\in\mathcal U_{\rm SV}$.
% All ``trace matrix'' objects below characterize only the Saint~Venant-equivalence class.
\end{definition}
...
% \section{Saint-Venant Motions}



\subsection{Class I: Kinematic Consistency}\label{sec:class-i}


It is natural to restrict attention to traces which exhibit the following properties:
\begin{enumerate}
    \item Strain measures \(\TraceStrain\) generated under Saint-Venant loading are at most linear in the axial coordinate $\reflenx$. 
    \item Any Saint-Venant strain field is exactly reconstructed from only the plane section strains \(\bm{\gamma},\bm{\kappa}\) \eqref{eq:gam-kap-linear}.
...
These considerations are formalized as follows.
...
\begin{enumerate}[label=(\roman*)]
    ...
    \begin{subequations}
    \begin{align}
        \TraceStrain'(x) & \eqqcolon \TraceEvolveEE\,\TraceStrain(0) 
        \label{eq:req-t1} 
        \\ 
        \partial_{\IesanVector}\TraceStrain'(x) &\eqqcolon \TraceDeDpOne 
        \label{eq:req-d1}
        \\ 
        \partial_{\TraceStrain} \snm(x) &\eqqcolon \TraceStiffNM 
        \label{eq:req-c1}
    \end{align}
    \label{eq:class-i-a}
    \end{subequations}
...
Strains \(\TraceStrain\) generated from a trace with the property \eqref{eq:class-i-a} can be written in the form:
\begin{equation}
\TraceStrain(x) 
