\section{The Section-Average Trace}
\label{sec:trace-cowper}

The section-average trace of a homogeneous section extracts the $L^2$-optimal rigid approximation 
...
\end{equation}
with \(\mathbf{1}\) the identity in \(\mathbb{R}^3\) and \(\CentroidLocation\in\Section\) is the centroid satisfying \(\SectionAverage{\bm{r}-\CentroidLocation} = \mathbf{0}\).
...
\end{equation*}
\end{proposition}
\input{Body/Traces/A3_CowperProofs}
...
+ \PlaneShape \Big) \bm{a}_\Omega,
\end{equation*}
where recall $\PlaneShape$ is the plane shape \eqref{eq:def-Theta}. 
The section averages give:
\begin{align*}
\bar{\bm{\theta}}^{(a)}(x) &= -x\,(\FrameBasis \times \bm{a}_\Omega) 
+ \RotationAverage{\PlaneShape}\,\bm{a}_\Omega.
\end{align*}
...
the shear strain is:
\begin{equation*}
\bar{\bm{\gamma}}^{(a)} = \FrameBasis \times \RotationAverage{\PlaneShape}\,\bm{a}_\Omega,
\end{equation*}
...
\end{equation}
The center of twist \eqref{eq:def-twist-center} has been attributed to \cite{trefftz1935uber} by \cite{pilkey2003analysis,romano2012torsion}.
...
\end{equation*}
where recall $\PlaneShearShape:\Section\rightarrow\Section\otimes\Section$ is the plane shear warping shape \eqref{eq:def-plane-shear-shape}. 
...
\end{equation*}
where with \(\FrameBasis\otimes\bm{\SectionLocation}_{\pi} := \RotationAverage{\PlaneShearShape}\):
\begin{subequations}
\begin{align}
% \mathbf{K}_1 &:= 
\TraceTF &:= \CenterShearLocation + \RotationAverage{\PlaneShearShape}^{\top}\FrameBasis
=: \CenterShearLocation + \bm{\SectionLocation}_{\pi}, 
\label{eq:cowper-TF} \\
% \TraceTF =: \CenterShearLocation + \FrameBasis^{\top}\RotationAverage{\PlaneShearShape}, 
% \\ %\label{eq:def-K1} \\
%
% \TraceMF 
\bm{\Gamma}_0
&:= \CenterTwistLocation \otimes \CenterShearLocation 
+ \RotationAverage{\FrameBasis \otimes \WarpShearShape{}}.
\label{eq:cowper-MF}
\end{align}
\end{subequations}
...
\begin{aligned}
\TraceFF 
&:= \SectionAverage{\PlaneShearShape} 
...
+ \CentroidLocation^\times \RotationAverage{\PlaneShearShape}\right)
...
\\[4pt]
\mathbf{0} 
& \mathbf{0} % (\FrameBasis \times \RotationAverage{\PlaneShape})_\perp 
& \FrameBasis \times \CenterTwistLocation & \TraceFF \\[4pt]
0 & \mathbf{0} & 1 & \TraceTF^\top %\CenterShearLocation^\top + (\RotationAverage{\PlaneShearShape})_\parallel^\top 
\\[4pt]
...
\label{eq:T0-cowper}
\end{equation}
where \(\TraceBT = \TraceNF = \mathbf{0}\). 
\begin{itemize}
...
\end{itemize}
In view of \eqref{eq:Tinv-from-T}:% \cref{tab:T0-inv-blocks}: %
\begin{equation}
\begin{array}{rlllll}
...
\ShiftScaleT & =1 - (\CenterShearLocation + \bm{\SectionLocation}_{\pi})\cdot\ShiftK 
 \\
\ShiftA &=-\FrameBasis^\times (\CenterTwistLocation \otimes \CenterShearLocation + \RotationAverage{\FrameBasis \otimes \WarpShearShape{}})\,\ShiftB 
& =\mathbf{0}
\\
\ShiftN && = \mathbf{0} 
 \\
...
& = 0
 \\
\end{array}
\label{eq:trace-blocks-geometric}
\end{equation}
...
\begin{table}[H]
    ...
    \(\bm{\SectionLocation}_{\pi}\) & \(\RotationAverage{\PlaneShearShape}^{\top}\FrameBasis\) \\
    \(\SectionAverage{\PlaneShearShape}\) & \(\tfrac12\Io\oneS - \IntRGoRG\) & & with \(\Io = \operatorname{tr}\IntRoR\)\\
    \bottomrule
    \end{tabular}
\end{table}
...
+ \begin{pmatrix} \mathbf{0}^\top \\ (\FrameBasis \times \RotationAverage{\PlaneShape})_\perp \\ \mathbf{0}^\top \\ -\FrameBasis^\times \end{pmatrix} \\
&= \begin{pmatrix} -\CentroidLocation^\top \\ (\FrameBasis \times \RotationAverage{\PlaneShape})_\perp \\ \mathbf{0}^\top \\ -\FrameBasis^\times \end{pmatrix}.
\end{align*}

The key observation is that $\RotationAverage{\PlaneShape}$ is parallel to 
$\FrameBasis$ (since $\bm{r} \times \PlaneShape$ lies in the axial direction), 
so $(\FrameBasis \times \RotationAverage{\PlaneShape})_\perp = \mathbf{0}$.
