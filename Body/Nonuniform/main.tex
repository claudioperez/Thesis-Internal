...
This phenomenon has been documented for torsional warping \cite{...} and examined in detail by \cite{armero2021modeling}. 
Within the enhanced strain framework of \cref{sec:enhan-section},
\[
\bar\varepsilon(\bm r;x)=\hat\varepsilon(\bm e;x)+\EnhanShapes(\bm r)\,\eta(x),
\]
the stationarity/orthogonality condition
\[
\residn(\eta)=\int_{\Section} \EnhanShapes^{\top}\StressVector\big(\hat\varepsilon+\EnhanShapes\eta\big)\,dA=0
\]
determines the section-local amplitude \(\eta(x)\). For a prescribed dimension \(n_a\), the modeling decision is the choice of an \(n_a\)-dimensional subspace
\[
\tilde{\mathscr E}=\{\EnhanShapes(\cdot)\eta:\eta\in\mathbb{R}^{n_a}\}
\subset L_2(A;\mathbb{R}^3),
\]
subject to the stability constraint \(\tilde{\mathscr E}\cap A[\mathcal E]=\{0\}\).

Outline:
\begin{itemize}
    \item \cref{sec:nonuniform-prelim}: preliminaries and weighted inner products.
    \item \cref{sec:nonuniform-residual}: derivation of the residual and its functional structure.
    \item \cref{sec:nonuniform-enhanced}: construction of the minimal compatible correction space.
...
\paragraph{Reduction.}
% Use the defining property \eqref{eq:def-sv-riesz} of \(\WarpTwistIesan\): 

% \begin{equation}
% \int_{\Section} G\,(\FrameBasis\times\bm{r}+\nabla\WarpTwistIesan)\cdot\nabla v\dA = 0
% \qquad\forall v.
% \label{eq:sv-property}
% \end{equation}
...
\begin{Note}
TODO: Motivate modeling residual strain by a gradient field. This can probably be argued as furnishing models that can be derived via stationarity.
\end{Note}
To cancel \eqref{eq:nonuniform-residual} by adding a \emph{compatible} correction shear field
\(\delta\tilde{\bm{\varepsilon}}_{\gamma}=\nabla\chi_c\), we require
...
% \[
% (\WarpTwistIesan,\WarpTwistIesan)_G=\GJv,\qquad (\psi,\psi)_G=\Js,\qquad (\psi,\WarpTwistIesan)_G=-\EJw,
% \]
...
= \GJv - 2\EnhanConstFA\,\EJw + \EnhanConstFA^2\Js.
\label{eq:chi-norm}
\end{equation}
%
Hence
...
=\frac{\GJv-\EnhanConstFA\EJw}{\GJv-2\EnhanConstFA\EJw+\EnhanConstFA^2\Js}.
...
\subsection{Cannonical Subspace}
...
\quad\Longleftrightarrow\quad
-\EJw+\EnhanConstFA \Js=0,
\]

hence

\begin{equation}
\EnhanConstFA=\frac{\EJw}{\Js}
...
\tightlist
\item
  \textbf{Gram--Schmidt \(G\)-orthogonalization:}


\[
(\chi_c,\WarpFA)_G = -\EJw + \EnhanConstFA\Js = 0.
\]

\item
  \textbf{Minimum \(G\)-norm of the potential \(\chi_c\):}

\[
\|\chi_c\|_G^2=\GJv-2\EnhanConstFA\EJw+\EnhanConstFA^2\Js
\ \ \text{is minimized at}\ \ 
\frac{\dd}{\dd \EnhanConstFA}\|\chi_c\|_G^2 = -2\EJw+2\EnhanConstFA\Js=0
\]
...
\[
\GJv-\frac{\tilde{J}(\EnhanConstFA)^2}{\|\chi_c\|_G^2}
=\frac{\EnhanConstFA^2\big(\GJv\Js-\EJw^2\big)}{\GJv-2\EnhanConstFA\,\EJw+\EnhanConstFA^2\Js}.
% \tag{6}
\]
...
\frac{\EnhanConstFA^2\big(\GJv\Js-\EJw^2\big)}{\GJv-2\EnhanConstFA\,\EJw+\EnhanConstFA^2\Js}=0.
...
\end{equation*}
where \(\EnhanConstFA\) is a constant of integration. 
...
\[\|\chi_c\|_G^2
= \Js\Big(\EnhanConstFA-\frac{\EJw}{\Js}\Big)^2+\Big(\GJv-\frac{\EJw^2}{\Js}\Big),\]

this is \(>0\) for all \(\EnhanConstFA\) provided the Gram determinant
\(\GJv\Js-\EJw^2>0\) (i.e.~\(\WarpTwistIesan\) and \(\WarpFA\) are
\(G\)-independent). 
For the degenerate case \(\GJv\Js=\EJw^2\), the energy \(\|\chi_c\|_G^2\)
vanishes at 
\[
\EnhanConstFA=\EJw/\Js
\]
...
Q = (\alpha-\Twist)\Big(\GJv-\frac{\tilde J(\EnhanConstFA)^2}{\|\chi_c\|_G^2}\Big)
   = (\alpha-\Twist)\,\frac{\EnhanConstFA^2(\GJv\Js-\EJw^2)}{\GJv-2\EnhanConstFA\EJw+\EnhanConstFA^2\Js}.
\]
