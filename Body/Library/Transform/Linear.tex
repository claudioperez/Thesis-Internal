...
After deformation, the endpoints have moved to \(\bm{X}_I + \bm{u}_I\)
 and \(\bm{X}_J + \bm{u}_J\). 
The direction of the line in the reference configuration $\Delta\bm{X}$, and in the deformed configuration $\Delta \bm{x}$ are
\begin{equation}
...
\end{equation}
%
The offset \(\bar{\bm{C}}\) is obtained using \eqref{eq:solve-cbar} for \(\bar{\bm{c}}\) in \eqref{eq:iso-u}:
\[
\begin{aligned}
\bar{\bm{u}}(\xi)
...
%
The objective is to obtain the derivative 
\[
...
Begin by obtaining the first order approximation of the unit
...
\(\Delta \bm{x}^*(\varepsilon)\). So \[
  ...
  \Delta \bm{x}^*(\varepsilon).
\] 
Differentiate both sides with respect to \(\varepsilon\). At
\(\varepsilon=0\), \(\bm{R}(0)=\mathbf{1}\) this furnishes:
\[
...
acts like a skew-symmetric operator on \(\Delta \bm{X}^*\). 
%
...
\begin{equation}
  ...
  =\bm{R}\frac{1}{L} \FrameBasis \times \Delta \bm{u}.
  }
\end{equation}
   ...
   Pull & & Push \\
    ...
    & {Node logarithm} & \\
        ...
        \end{pmatrix}
         \)
        \\
        \(\displaystyle\Big\downarrow\) & & \(\displaystyle\Big\uparrow\)
\end{tabular}
\end{center}
...
\begin{equation}
\begin{aligned}
L^{\prime} & =\frac{1}{L}\left(\bm{X}_J-\bm{X}_I\right) \cdot\left(\bm{X}_J^{\prime}-\bm{X}_I^{\prime}\right) \\
