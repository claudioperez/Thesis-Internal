...
The resulting formulations are implemented in \Xara{}, a runtime environment for the \OpenSees{} finite element framework that supports automatic differentiation through a hybrid adjoint approach.
...
\Cref{sec:transform} presents a framework for geometric transformations that ensures strain objectivity while preserving compatibility with standard element architectures.
...
\Cref{sec:timo} systematically investigates the optimal selection of warping modes with the objective of achieving exact representation of Saint~Veanant's continuum solutions through standard beam finite elements, without auxiliary warping fields.  
